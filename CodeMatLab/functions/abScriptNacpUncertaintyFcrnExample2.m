...
% 	addpath('i:/nacpUncertaintyExamples/CodeMatLab/'); 
	
	DirRoot='../'; 
	
	mkdir([DirRoot 'Output/FCRN/']); DirOut=[DirRoot 'Output/FCRN/']; 
...
%	Input one year of data at a time. 
	...
	for iFile=1:nFiles; 
		...
		disp(sprintf('abScriptNacpUncertaintyFcrnExample2 is processing file %g of %g, %s',iFile,nFiles,cSiteYr)); 
		disp(datestr(now)); 
		disp(' ' ); 
...
% % % 	uStarTh=mean(CpBoot); 
	...
	uStarTh=CpSine; nMC=1e2; iFig=106; % nMC is the # MonteCarlo reps
