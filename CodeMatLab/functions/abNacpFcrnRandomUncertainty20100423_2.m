	function [Ensemble,MatrixNEE,bNeg,bPos]=abNacpFcrnRandomUncertainty20100423_2 ...
		(t,NEE,uStar,Rsd,Ta,Ts,fNight,uStarTh,DirOut,cSiteYr,nMC,iFig,k,cFlux) 
...
% 	
		...
		aNEP(iMC)=k*sum(NEPgf); nMissNEP(iMC)=sum(isnan(NEPgf));
		aRE(iMC)=k*sum(REgf); nMissRE(iMC)=sum(isnan(REgf));
		aGPP(iMC)=k*sum(GPPgf); nMissGPP(iMC)=sum(isnan(GPPgf));
		
		stNEP(iMC,:)=k*NEPgf;
		stRE(iMC,:)=k*REgf;
		stGPP(iMC,:)=k*GPPgf;
	   
        sdNEP(iMC,:)=k*sum(reshape(NEPgf,nRecsPerDay,nd));
		sdRE(iMC,:)=k*sum(reshape(REgf,nRecsPerDay,nd));
		sdGPP(iMC,:)=k*sum(reshape(GPPgf,nRecsPerDay,nd));
	
        swNEP(iMC,:)=k*sum(reshape(NEPgf(itw),nRecsPerDay*7,nw));
		swRE(iMC,:)=k*sum(reshape(REgf(itw),nRecsPerDay*7,nw));
		swGPP(iMC,:)=k*sum(reshape(GPPgf(itw),nRecsPerDay*7,nw));
		
		for im=1:12;
			it=(find(m==im)); nit=length(it);
			smNEP(iMC,im)=k*sum(NEPgf(it));
			smRE(iMC,im)=k*sum(REgf(it));
			smGPP(iMC,im)=k*sum(GPPgf(it));
		end;
		...
		for im=1:12;
			it=(find(m==im)); nit=length(it);
			dmNEP(iMC,:,im)=mean(reshape(NEPgf(it),nRecsPerDay,nit/nRecsPerDay),2);
			dmRE(iMC,:,im)=mean(reshape(REgf(it),nRecsPerDay,nit/nRecsPerDay),2);
			dmGPP(iMC,:,im)=mean(reshape(GPPgf(it),nRecsPerDay,nit/nRecsPerDay),2);
		end;
