...
%   $Revision: 1.1.6.4 $  $Date: 2010/08/07 07:37:52 $
...
elseif strcmp(action,'run')
   ...
   dax = get(data_fig,'CurrentAxes');
   z = [24 90 155 215 285];
   x = 180 - 10*(m+1);
   row = [m+1 p1 p2 p3 y];
   for k = 1:5
       if k < 5,
          h(k) = text(z(k),x,sprintf('%4i',round(row(k))),'Parent',dax);
          set(h(k),'HorizontalAlignment','right');
          set(h(k),'FontName','Geneva','FontSize',fs);
       else
          h(k) = text(z(k),x,sprintf('%6.2f',row(k)),'Parent',dax);
          set(h(k),'HorizontalAlignment','right');
          set(h(k),'FontName','Geneva','FontSize',fs);
       end
...
elseif strcmp(action,'doe')
   ...
   for k = 1:13
     ...
     for k1 = 1:4
        h(k1) = text(z(k1),x,sprintf('%4i',round(row(k1))),'Parent',doe_axes);
     ...
     x = 160 - 10*k;
     row = [k p1(k) p2(k) p3(k) y(k)];
     h(5) = text(z(5),x,sprintf('%6.2f',row(5)),'Parent',doe_axes);
