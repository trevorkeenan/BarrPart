...
%   See also Predictor, Fitted, predict, random.
    ...
    methods(Abstract, Access='protected')
        % The predict method would normally take a dataset array or a matrix
    ...
    methods(Hidden, Access='public')
        function yPred = predictGrid(model,varargin)
            ...
            predData = dataset(gridCols{:},'VarNames',model.PredictorNames);
...
%
%    YPRED = FEVAL(M,DS) or YPRED = FEVAL(M,X) accepts a dataset DS or
...
%    See also Fitted, predict, LinearModel, GeneralizedLinearModel, NonlinearModel.
            ...
            if nargin-1 == npreds && ...% separate predictor variable arguments
               ~(nargin==2 && isa(varargin{1},'dataset'))
                predArgs = varargin;
                ...
                Xpred = dataset(asCols{:},'VarNames',model.PredictorNames);
                yPred = reshape(predict(model,Xpred),sizeOut);
            elseif nargin == 2 
                predVars = varargin{1};
                if isa(predVars,'dataset')
