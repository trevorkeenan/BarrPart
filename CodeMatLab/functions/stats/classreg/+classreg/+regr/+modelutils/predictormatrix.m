...
%PREDICTORMATRIX Construct predictor variable matrix from dataset array.
%   X = PREDICTORMATRIX(A) returns a numeric predictor variable matrix X
%   created from the variables in the NOBS-by-NVARS dataset array A.
...
%   $Revision: 1.1.6.2 $  $Date: 2011/10/15 02:05:51 $
...
if any(datasetfun(@ndims,data,'datavars',predictorVars) ~= 2)
...
ncols = datasetfun(@(x)size(x,2),data,'datavars',predictorVars);
ncols(charVars) = 1;
