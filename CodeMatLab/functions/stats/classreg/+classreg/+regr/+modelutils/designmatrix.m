...
%DESIGNMATRIX Construct regression design matrix from dataset array.
%   X = DESIGNMATRIX(A) returns a design matrix X created from variables in
%   the NOBS-by-NVARS dataset array A.  X is a design matrix for a linear
...
%      'CategoricalLevels' A cell array containing 
...
elseif isnumeric(model) && ismatrix(model)
    ...
    if length(interceptRow) > 1
        error(message('stats:classreg:regr:modelutils:MultipleIntercepts'));
    elseif ~supplied.Intercept
        % If a terms matrix is specified but the intercept flag is not, let
        % the terms determine whether or not an intercept is included
        includeIntercept = isscalar(interceptRow);
    elseif includeIntercept && isempty(interceptRow)
        % Add an intercept if requested, even if the terms matrix didn't have one
        terms = [zeros(nvars,1); terms];
    elseif ~includeIntercept && ~isempty(interceptRow)
...
if supplied.DummyVarCoding
    codingStrings = {'full' 'reference' 'referencelast' 'effects' 'difference' 'backwardDifference'};
    ...
    if ~haveStrs
        ...
        for i = 1:length(dummyCoding)
            if ~isempty(dummyCoding{i}) && ~any(strcmpi(dummyCoding{i},codingStrings))
                error(message('stats:classreg:regr:modelutils:UnrecognizedDummyCode',utils.listStrings(codingStrings)));
            end
        end
    end
end
...
if isDataset
    varClasses = datasetfun(@class,data,'datavars',predictorVars,'uniformoutput',false);
...
if nargin < 3 || isempty(glevels)
    ...
    if isa(group,'categorical')
        ...
        if length(uidx) < length(gn)
            ...
            [gidx,gn] = grp2idx(droplevels(group));
        end
    end
...
if isa(a,'categorical')
    if ischar(b)|| iscell(b)
        a = cellstr(a);
    elseif isa(b,'nominal')
        a = nominal(a,[],getlabels(b));
    elseif isa(b,'ordinal')
        a = ordinal(a,[],getlabels(b));
    else
