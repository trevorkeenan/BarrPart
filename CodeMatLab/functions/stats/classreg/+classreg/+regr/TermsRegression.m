...
%      History  Dataset representing the steps taken in the fit
%
%   The History dataset has one row for each step counting the initial fit,
    ...
    methods(Access='protected')
        ...
        function [design,coefTerm,coefNames] = designMatrix(model,X,isPredictorsOnly,terms)
            ...
            if isa(X,'dataset')
                ...
                [tf,varLocs] = ismember(X.Properties.VarNames,model.VariableNames);
                if ~all(tf)
        ...
        function model = assignData(model,X,y,w,asCat,dummyCoding,varNames,excl)
            ...
            if ~tf
                error(message('stats:classreg:regr:TermsRegression:BadCodingType'));
            elseif ~ismember(dummyCoding,classreg.regr.TermsRegression.DummyVarCodings)
                error(message('stats:classreg:regr:TermsRegression:BadCodingValue', internal.stats.listStrings( TermsRegression.DummyVarCodings )));
            end
        ...
        function fit = stepwiseFitter(start,nsteps,verbose)
            ...
            if isempty(start.Steps.History)
                history = dataset(nominal('Start',[],{'Start' 'Add' 'Remove'}), ...
                    {start.Formula.LinearPredictor}, ...
                    {terms}, start.NumEstimatedCoefficients,NaN,startReportedVals{:}, ...
                    'VarNames',[{'Action', 'TermName' 'Terms' 'DF' 'delDF'} reportedNames]);
            else
            ...
            while nsteps > 0 % nsteps might be inf
                ...
                if bestTestVal < addThreshold
                    ...
                    history(end+1,:) = dataset(nominal('Add'),addedTermName,{bestTerms},bestFit.NumEstimatedCoefficients,delDF,bestReportedVals{:});
                    if verbose
                    ...
                    if isnan(bestTestVal) || bestTestVal>removeThreshold
                        ...
                        delDF = fit.DFE - bestFit.DFE;
                        history(end+1,:) = dataset(nominal('Remove'),removedTermName,{bestTerms},bestFit.NumEstimatedCoefficients,delDF,bestReportedVals{:});
                        if verbose
        ...
        function formula = createFormula(supplied,modelDef,X, ...
            ...
            elseif givenString || givenTerms || givenAlias
                if haveDataset
                    ...
                    % not needed or accepted for a dataset array, because that has names.
...
% Get dataset variables that could be valid predictors

% Restrict predictors to specified var names
dsVarNames = X.Properties.VarNames;
if isempty(varNames)
