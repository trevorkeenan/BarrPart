classdef (Sealed = true) LinearFormula
...
%             '  MatlabExpr       = "[" [^#x5B#x5D]+ "]"' % [^#x5B#x5D] => anything but "[" or "]"
            ...
            '  Intercept        = "1"'
            '  Zero             = "0"'
           ['  AllElse          = "' classreg.regr.LinearFormula.allElseStr '"']
            '- Spaces           = (" ")*'
            };
    ...
    methods
        ...
        function f = set.Terms(f,update)
            if classreg.regr.LinearFormula.isTermsMatrix(update)
            ...
            elseif internal.stats.isString(update)
                [ustr,utree] = parseStr(update);
                if utree(1,1) ~= classreg.regr.LinearFormula.irules.LinearPredictor
                    error(message('stats:classreg:regr:LinearFormula:TermsBadLinearPredictor'));
                end
                f.terms = sortTerms(createTerms(utree,ustr,1,f.varNames));
            else
    ...
    methods(Access='public')
        function f = LinearFormula(modelSpec,varNames,responseVar,hasIntercept,link)
            ...
            if classreg.regr.LinearFormula.isTermsMatrix(modelSpec)
            ...
            elseif classreg.regr.LinearFormula.isModelAlias(modelSpec) ...
                    || (iscell(modelSpec) && classreg.regr.LinearFormula.isModelAlias(modelSpec{1}))
                ...
                elseif islogical(responseVar) && isvector(responseVar) && ...
                       sum(responseVar)==1 && length(responseVar)<=nvars
                    responseName = [];
            ...
            elseif internal.stats.isString(modelSpec)
                ...
                if f.tree(1,1) ~= classreg.regr.LinearFormula.irules.Formula
        ...
        function f = addTerms(f,update)
            ...
            elseif internal.stats.isString(update)
                ...
                [addTerms,removeTerms] = createTerms(utree,ustr,1,f.varNames,false);
...
%                 fstr = [fstr(1:i+2) '...'];
    ...
    methods(Access='protected')
        ...
        function [s,t] = substituteForAllElse(f,varNames)
            ...
            phLoc = find(t(1,:)==classreg.regr.LinearFormula.irules.AllElse);
            if isscalar(phLoc)
                if isMissingArg(varNames)
                    error(message('stats:classreg:regr:LinearFormula:BadAllElse', classreg.regr.LinearFormula.allElseStr));
                end
                ...
                if t(1,1) ~= classreg.regr.LinearFormula.irules.Formula
                     error(message('stats:classreg:regr:LinearFormula:InvalidFormula', modelString));
                end
            elseif ~isempty(phLoc)
                error(message('stats:classreg:regr:LinearFormula:MultipleAllElse', classreg.regr.LinearFormula.allElseStr));
            end
        end
        ...
        function f = processFormula(f,varNames)
            t = f.tree;
            s = f.str;
            if t(1,2) ~= classreg.regr.LinearFormula.irules.Response
                error(message('stats:classreg:regr:LinearFormula:ResponseAndPredictor'));
            end

            % Identify the link function, if any
            j = find(t(1,:)==classreg.regr.LinearFormula.irules.LinkFunction,1,'first');
            if isempty(j)
                f.link = 'identity';
            else
                j = j+1; % get just the function name
                if t(1,j)==classreg.regr.LinearFormula.irules.PowerLink
                    k = j + find(t(1,(j+1):end)==classreg.regr.LinearFormula.irules.Expon,1,'first');
                    expon = s(t(2,k):t(3,k));
            ...
            j = find(t(1,:) == classreg.regr.LinearFormula.irules.ResponseVar);
            f.responseName = s(t(2,j):t(3,j));

            % Identify any function calls
%             j = find(tree(1,:) == classreg.regr.LinearFormula.irules.FunName);
%             names = cell(size(j));
%             for i = 1:length(j)
...
%             f.FunctionCalls = uniqueLocal(names); % sorted
            ...
            f.terms = sortTerms(createTerms(t,s,lp,varNames));
            f.varNames = varNames;
            f.str = getStr(f);
        end
        ...
        function names = getPredictorAndResponseNames(f)
            t = f.tree;
            j = find((t(1,:) == classreg.regr.LinearFormula.irules.ResponseVar) | ...
                     (t(1,:) == classreg.regr.LinearFormula.irules.PredictorVar));
%             j = find((t(1,:) == classreg.regr.LinearFormula.irules.ResponseVar) | ...
%                      (t(1,:) == classreg.regr.LinearFormula.irules.MatlabExpr) | ...
%                      (t(1,:) == classreg.regr.LinearFormula.irules.PredictorVar));
            names = cell(size(j));
            for i = 1:length(j)
        ...
        function names = getPredictorNames(f)
            t = f.tree;
            j = find((t(1,:) == classreg.regr.LinearFormula.irules.PredictorVar));
%             j = find((t(1,:) == classreg.regr.LinearFormula.irules.PredictorVar) | ...
%                      (t(1,:) == classreg.regr.LinearFormula.irules.MatlabExpr));
            names = cell(size(j));
            for i = 1:length(j)
...
function [str,tree] = parseStr(str)
    ...
    str = strtrim(str);
    treestruct = classreg.regr.LinearFormula.p.parse(str);
...
function [addTerms,removeTerms] = createTerms(tree,str,start,vnames,addint)
    nvars = length(vnames);
    removeTerms = zeros(0,nvars);
    addTerms = identifyTerms(start+1);
    ...
    function [terms,node] = identifyTerms(node)
        switch tree(1,node)
        case classreg.regr.LinearFormula.irules.Sum
            last = node + tree(5,node) - 1;
            [terms,node] = identifyTerms(node+1);
            while node <= last
                terms1 = terms;
                type = tree(1,node);
                [terms2,node] = identifyTerms(node);
                switch type
                case classreg.regr.LinearFormula.irules.Addend
                    terms = [terms1; terms2];
                case classreg.regr.LinearFormula.irules.Subend
                    terms = terms1;
                end
            end
        case classreg.regr.LinearFormula.irules.Addend
        ...
        case classreg.regr.LinearFormula.irules.Subend
            terms = zeros(0,nvars);
            [rterms,node] = identifyTerms(node+1);
            removeTerms = [removeTerms; rterms];
        case classreg.regr.LinearFormula.irules.Product
            last = node + tree(5,node) - 1;
            [terms,node] = identifyTerms(node+1);
            while node <= last
                terms1 = terms;
        ...
        case classreg.regr.LinearFormula.irules.Inside
            [terms1,node] = identifyTerms(node+1);
            [terms2,node] = identifyTerms(node);
            n1 = size(terms1,1); n2 = size(terms2,1);
            i1 = repmat((1:n1)',1,n2); i2 = repmat(1:n2,n1,1);
            terms = [terms1; terms1(i1,:)+terms2(i2,:)];
        case classreg.regr.LinearFormula.irules.Interaction
            last = node + tree(5,node) - 1;
            [terms,node] = identifyTerms(node+1);
            while node <= last
                terms1 = terms;
        ...
        case classreg.regr.LinearFormula.irules.Power
            [terms2,node] = identifyTerms(node+1);
        ...
        case classreg.regr.LinearFormula.irules.PredictorVar
            ...
            node = node + 1;
        case classreg.regr.LinearFormula.irules.Intercept
            terms = zeros(1,nvars);
            node = node + 1;
        case classreg.regr.LinearFormula.irules.Zero
            terms = zeros(0,nvars);
            node = node + 1;
        otherwise
...
function str = prettyStr(str) % a stub
str(isspace(str)) = [];
str = regexprep(str,{'~' '+' '-'},{' ~ ' ' + ' ' - '});
end
