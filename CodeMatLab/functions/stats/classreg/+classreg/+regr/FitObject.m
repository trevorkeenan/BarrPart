classdef (AllowedSubclasses = {?classreg.regr.Predictor}) FitObject < classreg.learning.internal.DisallowVectorOps
...
%   $Revision: 1.1.6.8 $  $Date: 2012/05/08 20:42:52 $

    properties(GetAccess='public', SetAccess='protected')
%VariableInfo - Information about variables used in the fit.
%   The VariableInfo property is a dataset providing information about the
...
%
%   See also FitObject.
        VariableInfo = dataset({'double'},{[NaN; NaN]},false,false, ...
                               'VarNames',{'Class' 'Range' 'InModel'  'IsCategorical'}, ...
                               'ObsNames',{'y'});

%ObservationInfo - Information about observations used in the fit.
%   The ObservationInfo property is a dataset providing information about
%   the observations used in the fit. There is one row for each
%   observation. The dataset contains the following columns:
%
...
%
%   See also FitObject.
        ObservationInfo = dataset(zeros(0,1),false(0,1),false(0,1),false(0,1), ...
                                  'VarNames',{'Weights' 'Excluded' 'Missing' 'Subset'});
    end
    properties(GetAccess='protected', SetAccess='protected')
        PredLocs = zeros(1,0);
        RespLoc = 1;
        Data = []; % dataset(y,x1,x2,...) or struct('X',[],'y',[])
        IsFitFromData = false;
...
%Variables - Dataset of variables used in the fit.
%   The Variables property is a dataset containing data for the variables
%   used in the fit. This is a copy of the original dataset when the fit is
%   based on a dataset, or a dataset created from the predictor matrix and
%   response vector when the fit is based on those arrays.
%
%   See also FitObject.
        Variables
...
%   fit. This is the number of variables in the original dataset when the
%   fit is based on a dataset, or the total number of columns in the
%   predictor matrix and response vector when the fit is based on those
%   arrays. It includes variables, if any, that are not used as predictors
%   or as the response.
%
%   See also FitObject.
        NumVariables
...
%
%   If the fit is based on a dataset, this property provides the names of
%   the variables in that dataset.
%
%   If the fit is based on a predictor matrix and response vector, this
%   property is taken from the 'VarNames' argument supplied to the fit
%   method, if any. Otherwise the variables are given default names.
%
%   See also FitObject.
        VariableNames
...
%   dataset or matrix, minus any excluded rows or rows with missing (NaN)
%   values.
%
%   See also FitObject.
        NumObservations
...
%   dataset containing observation names, this property is taken from those
%   names. If the fit is based on a matrix or on a dataset without
...
%   See also FitObject.
    ...
    methods % get/set methods
        ...
        function vnames = get.VariableNames(model)
            vnames = model.VariableInfo.Properties.ObsNames;
        end
        function n = get.NumPredictors(model)
            n = length(model.PredictorNames);
        end
        function vnames = get.PredictorNames(model)
            vnames = model.VariableInfo.Properties.ObsNames(model.PredLocs);
        end
        function vname = get.ResponseName(model)
            vname = model.VariableInfo.Properties.ObsNames{model.RespLoc};
        end
        ...
        function onames = get.ObservationNames(model)
            if fitFromDataset(model)
                onames = model.Data.Properties.ObsNames; % possibly empty names
            else
                onames = {};
            end
        end
    ...
    methods(Access='protected')
        function model = noFit(model,varNames)
            ...
            model.VariableInfo = dataset(viClass,viRange,viInModel,viIsCategorical, ...
                                     'VarNames',{'Class' 'Range' 'InModel' 'IsCategorical'}, ...
                                     'ObsNames',viName);
            model.Data = dataset([{zeros(0,p+1)},varNames]);
        ...
        function model = assignData(model,X,y,w,asCat,varNames,excl)
            ...
            %    * in one dataset array X containing matrix or vector variables (and y is []),
            %    * in separate matrices X and y
            % We don't check types, we leave that for the selectVariables method.
            % varNames is required for data in X,y, ignored for data in a dataset array
            haveDataset = isa(X,'dataset');

            % Do some preliminary processing of the input data
            if haveDataset && all(datasetfun(@ismatrix,X))
                [nobs,nvars] = size(X);
                predLocs = 1:(nvars-1);
                respLoc = nvars;
            ...
            if haveDataset
                viName = X.Properties.VarNames;
                viClass = datasetfun(@class,X,'UniformOutput',false);
                
                % Provisionally choose the last variable as the response, and
                % the rest as predictors.
                viInModel = [true(1,nvars-1) false];
                
                viIsCategorical = datasetfun(@internal.stats.isDiscreteVar,X);
                ...
                data = X;
                obsNames = X.Properties.ObsNames;
            elseif ismatrix(X) && ismatrix(y)
            ...
            model.VariableInfo = dataset(viClass(:),viRange(:),viInModel(:),viIsCategorical(:), ...
                                     'VarNames',{'Class' 'Range' 'InModel' 'IsCategorical'}, ...
                                     'ObsNames',viName);
            model.ObservationInfo = dataset(w,excl,false(nobs,1),false(nobs,1), ...
                                     'VarNames',{'Weights' 'Excluded' 'Missing' 'Subset'},...
                                     'ObsNames',obsNames);
        end
        ...
        function model = updateVarRange(model)
            ...
            if fitFromDataset(model)
                vnames = model.Data.Properties.VarNames;
        ...
        function model = selectVariables(model)
            ...
            if haveDataset
                data = model.Data;
                isNumVar = datasetfun(@isnumeric,data);
                isNumVec = isNumVar & datasetfun(@isvector,data);
                isCatVec = datasetfun(@internal.stats.isDiscreteVec,data);
        ...
        function vars = getVariables(model,iobs,varargin) % dependent property get/set method override
            % get a dataset containing all or some of the fit's variables,
            % possibly only a subset of the observations.  iobs and jvars can
            % be anything that dataset subscripting supports.
            if nargin < 2
                iobs = ':';
            ...
            if fitFromDataset(model)
                ...
                vars = dataset(Xvars{:},model.Data.y(iobs,:),'VarNames',model.VariableNames);
                vars = vars(:,varargin{:});
            end
        end
        ...
        function [varData,varName,varNum] = getVar(model,var)
            ...
            if any(t)
                if isnumeric(varData)
                    varData(t) = NaN;
                elseif isa(varData,'nominal')
                    varData(t) = nominal(NaN);
                elseif isa(varData,'ordinal')
                    varData(t) = ordinal(NaN);
                end
            end
        end
        ...
        function [Xeval,respSz] = preEval(model,toMatrix,varargin)
            ...
            % numeric matrix or dataset array containing all the variables as
            % columns, in separate numeric arrays (one per predictor) with
            % common shape.
            p = model.NumPredictors;
            if isscalar(varargin) && isa(varargin{1},'dataset') % data in a dataset array
                Xeval = varargin{1};
                [tf,predLocs] = ismember(model.PredictorNames,Xeval.Properties.VarNames);
                if ~all(tf)
                ...
                if toMatrix
                    Xeval = datasetfun(@(x) x, Xeval, 'DataVars',predLocs, 'UniformOutput',false);
                    Xeval = cat(2,Xeval{:});
                end
            elseif (nargin-2) == p
    ...
    methods(Hidden, Access='public')
        ...
        function [varargout] = subsref(a,s)
            ...
            case '.'
                ...
                elseif any(strcmp(s(1).subs,properties(a)))
                    [varargout{1:nargout}] = builtin('subsref',a,s);
                elseif any(strcmp(s(1).subs,methods(a)))
                    [varargout{1:nargout}] = builtin('subsref',a,s);
                else
...
if isa(X,'dataset') && internal.stats.isString(w) % a dataset variable name
    [tf,wloc] = ismember(w,X.Properties.VarNames);
    if ~tf
