...
%   $Revision: 1.1.6.5.2.1 $  $Date: 2012/06/22 15:31:54 $
...
if ~isequal(cvp,'resubstitution')
    ...
    cvfun = @(Xtrain,Ytrain,Xtest,Ytest) lassoFitAndPredict( ...
        ...
        lambda,alpha,P,standardize,reltol,lambdaMax*totalWeight,ever_active, ...
        penalizedFitPartition,distr,link,linkFun,sqrtvarFun,devFun,dataClass, ...
        nullDev,nullIntercept);
    weights = pwts;
    if isempty(weights)
...
function dev = lassoFitAndPredict(Xtrain,Ytrain,Xtest,Ytest, ...
    lambda,alpha,dfmax,standardize,reltol,lambdaMax,ever_active, ...
    penalizedFitPartition,distr,link,linkFun,sqrtvarFun,devFun,dataClass, ...
    nullDev,nullIntercept)
% This function sets up the conditions for lassoFit and prediction from
...
% Note: lasso bypasses fits with user-supplied lambda if lambda>lambdaMax.
% In lassoglm we don't do this.
regressionType = 'GLM';
