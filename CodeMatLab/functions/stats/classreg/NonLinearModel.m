...
%   NonLinearModel represents a regression model for predicting a response as
%   a nonlinear function of predictor variables. You create a
%   NonLinearModel by using the NonLinearModel.fit function to fit the
%   model to data.
%
%   NonLinearModel methods:
%       fit - Fit nonlinear model to data (static)
%
...
%
%   See also LinearModel, GeneralizedLinearModel.
...
%   The WeightedResiduals property is a table of residuals. It is a dataset 
...
%   the NonLinearModel NLM using predictor variables from the dataset DS. DS
...
%      % observations
%      load reaction
%      nlm = NonLinearModel.fit(reactants,rate,@hougen,[1 .05 .02 .1 2])
...
%   NonLinearModel NLM using predictor variables from the dataset DS. DS
...
%      load census
%      start = [1 1 1900 1];
%      nlm = NonLinearModel.fit(cdate,pop,'pop ~ b1 + b2*abs(cdate-b3)^b4',start)
%
...
%      nlm = NonLinearModel.fit(reactants,rate,@hougen,...
...
%   See also LinearModel, GeneralizedLinearModel, NonLinearModel, linhyptest.
    ...
    methods(Access='protected')
        ...
        function table = tstats(model)
            ...
            % (2) Get the contents of dataset table and replace columns 2-4 
            ...
            tableVals = double(table);
            tableVals(~estimable,2:4) = NaN;
            table = replacedata(table,tableVals);                     
        end
       ...
       % "overloaded" get functions
        function D = get_diagnostics(model,type)
            if nargin<2 % return all diagnostics in a dataset
                ...
                D = dataset({model.Leverage,'Leverage'},CooksDistance,HatMatrix,...
        ...
        function design = predictorMatrix(model,X,isPredictorsOnly)
            if isa(X,'dataset')
                % Given a dataset, make sure it has all of the predictor
                % variables.  It can also have any unused variables from
                % the model fit.
                tf = ismember(model.PredictorNames,X.Properties.VarNames);
                if ~all(tf)
        ...
        function L = getlogLikelihood(model)
            ...
            if isempty(model.Robust)
                ...
                L = -(model.DFE + model.NumObservations*log(2*pi) + sum(log(model.MSE./w_r)))/2;
            else
                ...
                y = getResponse(model);
                y = y(subset);
                L = -( sum((y-yfit).^2)/model.MSE + model.NumObservations*log(2*pi) + model.NumObservations*log(model.MSE) )/2;
            end
        ...
        function L0 = logLikelihoodNull(model)
            ...
            %   nlm0 = NonLinearModel.fit(ds,linfun0,rand);
        ...
        function r = get_weighted_residuals(model,type)            
            ...
            if isa(r,'dataset')
                r = datasetfun(@(xx) xx .* sqrt(w), r, 'UniformOutput',false, 'DatasetOutput',true);
            else
                r = sqrt(w) .* r;
            end   
        end
...
%FIT Create nonlinear regression model by fitting to data.
%   NLM = NonLinearModel.fit(DS,MODELFUN,BETA0) fits the model specified
%   by MODELFUN to variables in the dataset array DS, and returns the
%   nonlinear model NLM. The coefficients are estimated using an iterative
%   procedure starting from the initial values in the vector BETA0.
%
...
%        dataset are taken to be predictors, and the others are
%        coefficients to be estimated.
%
%   NLM = NonLinearModel.fit(X,Y,MODELFUN,BETA0) fits a nonlinear
%   regression model using the column vector Y as a response variable and
%   the columns of the matrix X as predictor variables.
%
%   NLM = NonLinearModel.fit(...,BETA0,PARAM1,VAL1,PARAM2,VAL2,...)
...
%                         created by calling statset('nlinfit').
%
%   Example:
...
%      nlm = NonLinearModel.fit(reactants,rate,myfun,[1 .05 .02 .1 2])
%
%   See also NonLinearModel, statset, nlinfit.
        ...
        function formula = createFormula(supplied,modelDef,X,ncoefs,coefNames,predictorVars,responseVar,varNames,haveDataset)
            ...
            elseif givenString || givenFun
                ...
                if haveDataset
                    ...
                    % not needed or accepted for a dataset array.
                    ...
                    % ResponseVar is intended to say which variable in a dataset array is
