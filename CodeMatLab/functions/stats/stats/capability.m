...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:24:18 $

if nargin~=2
    error(message('stats:capability:WrongNumberOfInputs'));
end


if ~isnumeric(data) || ndims(data)>2
   error(message('stats:capability:BadData'));
end
if isvector(data)
    data = data(:);
end
...
if m==1
    if ~isnumeric(specs) || numel(specs)~=2
        error(message('stats:capability:BadSpecs1'));
    end
    specs = specs(:);
else
    ...
    elseif ~isnumeric(specs) || ~isequal(size(specs),[2 m])
        error(message('stats:capability:BadSpecs2', sprintf( '%d', m )));
    end  
end
...
if any(isinf(lb) & isinf(ub))
   error(message('stats:capability:BadSpecs3'));
elseif any(lb==ub)
   error(message('stats:capability:BadSpecs4'));
end
