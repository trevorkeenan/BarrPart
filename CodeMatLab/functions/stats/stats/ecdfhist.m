...
%   $Revision: 1.1.8.4 $  $Date: 2011/07/20 00:08:23 $
...
if nargs<2
    error(message('stats:ecdfhist:TooFewInputs'));
end

% Inputs should look like they came from ecdf.
if ischar(F) || ischar(X)
    error(message('stats:ecdfhist:NotNumeric'))
end
if numel(F)~=length(F) || numel(X)~=length(X)
    error(message('stats:ecdfhist:VectorRequired'));
elseif length(F)<2 || length(X)<2
    error(message('stats:ecdfhist:TooFewElements'));
elseif any(diff(X)<0)
    error(message('stats:ecdfhist:NotSorted'));
end
...
if any(diff(F)<0)
    if wassurv
        error(message('stats:ecdfhist:NotSurvivor'))
    else
        error(message('stats:ecdfhist:NotCdf'))
    end
end
...
if nargout == 0
    ...
    if nargout>=2
        Cout = C;
        if wng
            warning(message('stats:ecdfhist:InconsistentEdges'));
        end
    end
end
