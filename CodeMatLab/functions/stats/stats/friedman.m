...
%   $Revision: 1.1.8.4 $  $Date: 2011/08/29 20:38:24 $
...
if (any(isnan(X(:))))
   error(message('stats:friedman:NanNotAllowed'));
end
if (reps>1)
   r = r/reps;
   if (floor(r) ~= r), 
       error(message('stats:friedman:BadSize1'));
   end
end   
if (r<=1 | c <=1)
   error(message('stats:friedman:BadSize2'));
end
if ~(isequal(displayopt,'on') | isequal(displayopt,'off'))
   error(message('stats:friedman:BadDisplayOpt'));
end
...
p = chi2pval(chistat, c-1);

Table(3,:) = [];                    % remove row info
if (reps>1), Table{3,5} = []; Table{3,6} = []; end % remove interaction test
Table{1,5} = getString(message('stats:friedman:ColHeadChisq'));       % fix test statistic name
Table{2,5} = chistat;                                                 % fix test statistic value
Table{1,6} = getString(message('stats:friedman:ColHeadProbGtChisq')); % fix p-value name
Table{2,6} = p;                                                       % fix p-value value

if (isequal(displayopt, 'on'))
   digits = [-1 -1 0 -1 2 4];
   statdisptable(Table, getString(message('stats:friedman:TableTestName')), getString(message('stats:friedman:TableName')), ...
              getString(message('stats:friedman:TableComment')), digits);
end
