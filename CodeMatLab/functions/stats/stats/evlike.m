...
%   DATA.  If Y has a Weibull distribution, then X=log(Y) has the type 1
...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:24:53 $

if nargin < 2
    error(message('stats:evlike:TooFewInputs'));
elseif numel(data) > length(data)
    error(message('stats:evlike:VectorRequired'));
end
...
elseif ~isequal(size(data), size(censoring))
    error(message('stats:evlike:DataCensSizeMismatch'));
end
...
elseif isequal(size(data), size(freq))
    ...
    error(message('stats:evlike:DataFreqSizeMismatch'));
end
