...
%   $Revision: 1.1.6.2 $  $Date: 2010/10/08 17:22:15 $

if nargin < 2
    error(message('stats:betalike:TooFewInputs'));
elseif ~isvector(data)
    error(message('stats:betalike:VectorRequired'));
end
x = data(:);
...
if nargout > 1
    if numel(data) < 2
        error(message('stats:betalike:NotEnoughData'));
    end
