...
if nargin < 2 && isvector(y)
    error(message('stats:multivarichart:TooFewInputs'));
end
...
if mod(nargin,2)==1 && size(y,2)>1 % matrix data
    [varnames,plotorder] = internal.stats.parseArgs(args,defaults,varargin{:});
else
    group = varargin{1};
    [varnames,plotorder] = internal.stats.parseArgs(args,defaults,varargin{2:end});
end;

if ischar(plotorder) % string PLOTORDER
    plotorder = lower(plotorder);
    if  ~strcmp(plotorder, 'sorted') && ~strcmp(plotorder, 'unsorted')  
        error(message('stats:multivarichart:BadPlotorder'));
    end
end;
...
if ~iscellstr(varnames) && ~ischar(varnames)
    error(message('stats:multivarichart:BadVarnames'));
end
...
if mod(nargin,2)==1 && isvector(y)
    error(message('stats:multivarichart:BadData'));
end
...
for i = 1:ng
    if isnumeric(group{i})
        if ~isvector(group{i})
           error(message('stats:multivarichart:BadGroupingVar'));
        end
    end;
...
if  any(cellfun(@length,group)~=size(y,1))
        error(message('stats:multivarichart:GroupingVarSizeMismatch'));
end;
...
switch ng
    case 1  % only one grouping factor
        error(message('stats:multivarichart:TooFewFactors'));
    ...
    otherwise % no more than 4 grouping factors are allowed.
        error(message('stats:multivarichart:TooManyFactors'));
end;
...
if ishghandle(container) && isequal(get(container,'Type'),'uicontainer') ...
        && ~isequal(get(evt.AffectedObject,'BeingDeleted'),'on')
