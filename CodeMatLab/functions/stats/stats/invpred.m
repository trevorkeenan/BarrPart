...
%   $Revision: 1.1.8.4 $  $Date: 2011/12/27 16:17:00 $

narginchk(3,inf);
% Check data inputs
if ~isvector(x) || ~isreal(x)
    error(message('stats:invpred:BadX'))
end
if ~isvector(y) || ~isreal(y)
    error(message('stats:invpred:BadY'))
end
if numel(x) ~= numel(y)
    error(message('stats:invpred:InputSizeMismatch'));
end
x = x(:);
y = y(:);
t = isnan(x) | isnan(y);
...
if ~isreal(y0)
    error(message('stats:invpred:BadY0'))
end

% Process optional inputs and check them
okargs =   {'alpha' 'predopt'};
defaults = {0.05    'obs'};
[alpha predopt] = internal.stats.parseArgs(okargs,defaults,varargin{:});

if ~isscalar(alpha) || ~isnumeric(alpha) || ~isreal(alpha) ...
                    || alpha<=0          || alpha>=1 ...
                    || isnan(alpha)
    error(message('stats:invpred:BadAlpha'));
end
i = find(strncmpi(predopt,{'curve';'observation'},length(predopt)));
if ~isscalar(i)
    error(message('stats:invpred:BadPredOpt'));
end
doobs = (i==2);
...
normx = norm(x)^2;
if normx==0
    error(message('stats:invpred:ConstantX'))
end
