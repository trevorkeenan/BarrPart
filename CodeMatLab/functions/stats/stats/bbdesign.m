...
%   $Revision: 1.1.8.4 $  $Date: 2011/07/20 00:08:09 $

okargs = {'center' 'blocksize'};
defaults = {[] Inf};
[ncenter,blocksize] = internal.stats.parseArgs(okargs,defaults,varargin{:});

if ~isnumeric(nfactors) || ~isscalar(nfactors)
   error(message('stats:bbdesign:NumFactorsNotNumericOrScalar'));
elseif nfactors<3 || nfactors~=floor(nfactors)
   error(message('stats:bbdesign:NumFactorsNotIntegerOrLessThanTwo', nfactors));
end
...
if numel(ncenter)~=1 || ncenter<0 || ncenter~=floor(ncenter)
   error(message('stats:bbdesign:BadCenter'));
end

% Check block size
if blocksize ~= Inf
   if ~isnumeric(blocksize) || blocksize<1 || blocksize~=floor(blocksize)
      error(message('stats:bbdesign:BadBlockSize'));
   end
end
...
if nargout>=2
   ...
   if ctrwng
      warning(message('stats:bbdesign:CenterPointDeficit', ncenter, max( blk )));
   end
end
...
if nargout>=2
   ...
   if b0>blocksize
      warning(message('stats:bbdesign:BlockSizeExceeded', b0, blocksize));
   end
end
