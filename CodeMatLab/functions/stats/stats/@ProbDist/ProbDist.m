...
%   $Revision: 1.1.8.4 $  $Date: 2011/07/20 00:09:05 $
...
%   See also PROBDIST, FIT.
    ...
    methods(Hidden = true)
        ...
        function varargout = empty(varargin)
            error(message('stats:ProbDist:NoEmpty'))
        end
...
error(message('stats:ProbDist:NoCatAllowed'));
end

function throwUndefinedError
st = dbstack;
name = regexp(st(2).name,'\.','split');
error(message('stats:ProbDist:UndefinedFunction', name{ 2 }, name{ 1 }));
end
