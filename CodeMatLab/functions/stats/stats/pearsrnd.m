...
%   $Revision: 1.1.8.2 $  $Date: 2010/10/08 17:26:02 $

if nargin < 4
    error(message('stats:pearsrnd:TooFewInputs'));
elseif ~(isscalar(mu) && isscalar(sigma) && isscalar(skew) && isscalar(kurt))
    error(message('stats:pearsrnd:NonScalarInputs'));
end

[err, sizeOut] = statsizechk(4,mu,sigma,skew,kurt,varargin{:});
if err > 0
    error(message('stats:pearsrnd:InputSizeMismatch'));
end
