...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:26:30 $
...
if nargout > 1
    ...
    elseif ~isscalar(alpha) || ~isnumeric(alpha) || ~isreal(alpha) ...
                            || alpha<=0          || alpha>=1
        error(message('stats:polyconf:BadAlpha'));
    end
    if isempty(mu)
