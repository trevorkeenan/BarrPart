...
%   $Revision: 1.1.8.4 $  $Date: 2011/07/20 00:08:25 $
...
elseif ~isequal(size(x), size(censoring))
    error(message('stats:gamfit:InputSizeMismatchCensoring'));
end
...
elseif isequal(size(x), size(freq))
    ...
    error(message('stats:gamfit:InputSizeMismatchFreq'));
end
...
if min(size(x)) > 1
    error(message('stats:gamfit:VectorRequired'));

% Illegal data return an error.  Zeros are allowed when there is no censoring.
elseif ncen == 0 && any(x < 0)
    error(message('stats:gamfit:BadDataNonNeg'));
elseif ncen > 0 && any(x <= 0)
    error(message('stats:gamfit:BadDataPosCens'));
end
...
if ncen == 0
    ...
    % Otherwise, find MLEs.
        ...
        if lkeqn(ahat, const) > 0
            ...
            while lkeqn(lower, const) > 0
                ...
                if lower < realmin(class(x)) % underflow, no positive root
                    error(message('stats:gamfit:NoSolution'));
                end
            end
        else
        ...
        elseif eps(abs(lkeqnval)) > options.TolX
            warning(message('stats:gamfit:IllConditioned'));
        end
...
% strictly positive, with a full-length censoring vector.
    ...
    if (err == 0)
        ...
        if output.funcCount >= options.MaxFunEvals
            warning(message('stats:gamfit:EvalLimit'));
        else
            warning(message('stats:gamfit:IterLimit'));
        end
    elseif (err < 0)
        error(message('stats:gamfit:NoSolution'));
    end
