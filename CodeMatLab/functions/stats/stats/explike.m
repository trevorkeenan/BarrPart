...
%   for the exponential distribution, evaluated at the mean parameter
...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:24:57 $

if nargin < 2
    error(message('stats:explike:TooFewInputs'));
elseif numel(data) > length(data)
    error(message('stats:explike:VectorRequired'));
end
...
elseif ~isequal(size(data), size(censoring))
    error(message('stats:explike:InputSizeMismatchCensoring'));
end
...
elseif isequal(size(data), size(freq))
    ...
    error(message('stats:explike:InputSizeMismatchFreq'));
end
