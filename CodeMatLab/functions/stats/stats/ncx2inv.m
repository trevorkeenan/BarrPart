...
%   $Revision: 1.1.8.4 $  $Date: 2011/10/15 02:06:35 $

if nargin<3
    error(message('stats:ncx2inv:TooFewInputs'));
end
[errorcode p v delta] = distchck(3,p,v,delta);

if errorcode > 0
    error(message('stats:ncx2inv:InputSizeMismatch'));
end
...
if count == count_limit
    warning(message('stats:ncx2inv:NotConverge'));
    str = getString(message('stats:ncx2inv:LastStep'));
