...
%   $Revision: 1.1.8.13 $  $Date: 2012/04/02 22:16:48 $
...
while i <= nargin
    ...
    if ~expectval
        ...
        [~,j] = internal.stats.getParamVal(arg,lowNames,'parameter name');
        name = names{j};
...
%   CHECKPARAM('name',VALUE) checks that the specified value VALUE is valid
...
switch name
    ...
    case {'WgtFun'}
        ...
        if ~strcmp(class(value), 'function_handle')&&(~ischar(value) || ~any(strcmpi(value,values)))
