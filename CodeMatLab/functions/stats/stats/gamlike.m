...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:25:07 $

if nargin < 2
    error(message('stats:gamlike:TooFewInputs'));
elseif numel(data) > length(data)
    error(message('stats:gamlike:VectorRequired'));
end
...
elseif ~isequal(size(data), size(censoring))
    error(message('stats:gamlike:InputSizeMismatchCensoring'));
end
...
elseif isequal(size(data), size(freq))
    ...
    error(message('stats:gamlike:InputSizeMismatchFreq'));
end
...
if nargout == 2
    dL11 = -psi(1,a) * ones(size(z));
    dL12 = -(1./b) * ones(size(z));
    dL22 = -(2.*z-a) ./ (b.^2);
    if ncen > 0
