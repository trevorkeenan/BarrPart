...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:24:23 $

if ~isscalar(nfactors) || ~isnumeric(nfactors) || nfactors<2 ...
                       || nfactors>26 || nfactors~=round(nfactors)
   error(message('stats:ccdesign:BadNumFactors'))
end
okargs = {'center' 'fraction' 'type' 'blocksize'};
defaults = {[] [] 'circumscribed' Inf};
[ncenter,frac,startype,blocksize] = ...
...
if isempty(startype) || ~ischar(startype)
   ...
   i = find(strncmpi(startype, oktypes,length(startype)));
end
if isempty(i)
   error(message('stats:ccdesign:BadType'));
end
startype = oktypes{i};
if blocksize ~= Inf
   if ~isnumeric(blocksize) || blocksize<1 || blocksize~=floor(blocksize)
      error(message('stats:ccdesign:BadBlockSize'));
   end
end
...
if isempty(g)
   error(message('stats:ccdesign:NoDesign', nfactors, frac));
end
nc = 2^(nfactors-frac);
...
if ischar(ncenter) && ~isempty(ncenter)
   ...
   j = find(strncmpi(ncenter,okvals,length(ncenter)));
   if isempty(j)
      error(message('stats:ccdesign:BadCenter'));
   end
   ncenter = okvals{j};
end
useorth = false;
if isequal(ncenter,'uniform')
   ...
   if useorth
      warning(message('stats:ccdesign:NoUniformPrecision'));
   end
end
if isempty(ncenter) || isequal(ncenter,'orthogonal') || useorth
   % See Box & Hunter (1957), eqn. 81, with lambda4=1.
   ncenter = max(1,ceil(4*sqrt(nc) + 4 - 2*nfactors));
elseif ~isscalar(ncenter) || ncenter<0 || ncenter~=floor(ncenter)
   error(message('stats:ccdesign:BadCenter'));
end
...
if blocked
   ...
   if minblksize>blocksize
      warning(message('stats:ccdesign:BlockSizeExceeded', minblksize));
   end
   ...
   if ncenter>length(ctrblks)
      warning(message('stats:ccdesign:CenterPointLimit', length( ctrblks )));
      ncenter = length(ctrblks);
   elseif ncenter<nblocks
      warning(message('stats:ccdesign:CenterPointDeficit', ncenter, nblocks));
   end
end
