...
%
%   For example, the following matrix X and model MODEL produce the design
%   matrix D: 
%            X = [1 10     MODEL = [0 0     D = [1 1 10 10  1
...
%
%   Example: For a matrix X with three columns, compute terms of a
%            polynomial of up to third order (X1^3, X2*X3^2, etc.)
%      x = [rand(5,1), randn(5,1), randi(4,5,1)];
%      m = fullfact([4 4 4])-1;  % all combinations of 0,1,2,3
%      m(sum(m,2)>3,:) = [];     % remove ones with order >3
%      d = x2fx(x,m);            % compute design matrix
% 
%   See also RSTOOL, REGSTATS, CANDEXCH, CANDGEN, CORDEXCH, ROWEXCH. 

%   Copyright 1993-2006 The MathWorks, Inc.
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:27:19 $
   ...
   % Make sure all categorical variables take valid values
    ...
    for j=1:length(categ)
        ...
        if any(~ismember(x(:,cj),1:catlevels(j)))
           error(message('stats:x2fx:Categories', cj, catlevels( j )));
        end
    end
end
...
if ischar(model)
   ...
   [row,col] = size(model);
   if col ~= n
      error(message('stats:x2fx:BadSize'));
   end
   if any(model(:,categ)>1,1)
      error(message('stats:x2fx:BadModel'));
   end      
