...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:27:58 $

switch s(1).type
    case '()'
        % No cascaded subscripts are allowed to follow parenthesis indexing.
        if ~isscalar(s)
            error(message('stats:classregtree:subsref:InvalidSubscriptExpr'));

        elseif numel(s(1).subs) ~= 1
            error(message('stats:classregtree:subsref:ParenArgNotSingleArray'));
        end
    ...
    case '{}'
        error(message('stats:classregtree:subsref:CellSubscript'));
    ...
    case '.'
        ...
        if ismember(s(1).subs,methodnames)
            ...
            elseif numel(s)>2 || ~isequal(s(2).type,'()')
                error(message('stats:classregtree:subsref:DotSubscriptWithNonScalarsNotAllowed'));
            elseif numel(s)==2
                args = s(2).subs;
            end
