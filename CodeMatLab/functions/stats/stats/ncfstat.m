...
%   $Revision: 1.1.8.2 $  $Date: 2010/10/08 17:25:31 $

if nargin < 2, 
    error(message('stats:ncfstat:TooFewInputs')); 
end

[errorcode, nu1, nu2, delta] = distchck(3,nu1,nu2,delta);

if errorcode > 0
    error(message('stats:ncfstat:InputSizeMismatch'));
end

% Initialize the mean and variance to zero.
if isa(nu1,'single') || isa(nu2,'single') || isa(delta,'single')
   m = zeros(size(delta),'single');
else
   m = zeros(size(delta));
end
v = m;
