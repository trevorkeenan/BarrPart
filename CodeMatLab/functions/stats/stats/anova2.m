...
%   for testing  column, row, and if possible interaction effects.  TABLE
...
%   $Revision: 1.1.8.3 $  $Date: 2011/08/29 20:38:10 $
...
if (SSE~=0)
    ...
    colf = (CSS/df1) / MSE;
    rowf = (RSS/df2) / MSE;
    colp = fpval(colf,df1,edf);  % Probability of F given equal column means.
    rowp = fpval(rowf,df2,edf);  % Probability of F given equal row means.
    p    = [colp rowp];

    if (reps > 1),
       intf = (ISS/idf)/MSE;
       ip   = fpval(intf,idf,edf);
       p    = [p ip];
    end
...
if (reps > 1),
  ...
  Table(2:6,1)={getString(message('stats:anova2:RowHeadColumns'));getString(message('stats:anova2:RowHeadRows'));getString(message('stats:anova2:RowHeadInteraction'));getString(message('stats:anova2:RowHeadError'));getString(message('stats:anova2:RowHeadTotal'))};
  ...
  Table(2:5,1)={getString(message('stats:anova2:RowHeadColumns'));getString(message('stats:anova2:RowHeadRows'));getString(message('stats:anova2:RowHeadError'));getString(message('stats:anova2:RowHeadTotal'))};
...
Table(1,1:6) = {getString(message('stats:anova2:ColHeadSource')) getString(message('stats:anova2:ColHeadSS')) getString(message('stats:anova2:ColHeadDf')) getString(message('stats:anova2:ColHeadMS')) getString(message('stats:anova2:ColHeadF')) getString(message('stats:anova2:ColHeadProbGtF'))};
Table(2:(1+length(p)),6) = num2cell(p);

if (isequal(displayopt, 'on'))
   digits = [-1 -1 0 -1 2 4];
   statdisptable(Table, getString(message('stats:anova2:TwoWayANOVA')), getString(message('stats:anova2:ANOVATable')), '', digits);
end
