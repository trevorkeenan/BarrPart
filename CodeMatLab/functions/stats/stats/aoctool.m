...
%   (anocova) using X as the predictor, Y as the response, and G as the
%   grouping variable.  X and Y must be vectors.  G may be a categorical
%   variable, vector, character array, or cell array of strings.  AOCTOOL
%   displays three figures; one contains an interactive graph of the data
%   and prediction curves, another contains an anova table, and a third
%   contains parameter estimates.  The graph figure can display predictions
%   for all groups, or predictions and confidence bounds for a single
%   group.  The confidence level is 100(1-ALPHA) percent, with ALPHA=0.05
%   as the default.
%
...
%   $Revision: 1.1.8.7 $  $Date: 2012/03/01 02:29:55 $

action = 'start';
nargs = nargin;
if (nargs > 0), if (ischar(x)), action = x; end; end

%On initial call check for arguments, then weed out NaNs
if (strcmp(action, 'start'))
   noargmsg=[];
   if (nargs == 0)
      noargmsg = getString(message('stats:aoctool:NoArgMsg'));
      s = load('carsmall');
      ...
      g = s.Model_Year;
      alpha = 0.05;
      xname = getString(message('stats:aoctool:Weight'));
      yname = getString(message('stats:aoctool:MPG'));
      gname = getString(message('stats:aoctool:ModelYear'));
      nargs = 7;
   end

   if (nargs < 3)
      error(message('stats:aoctool:TooFewInputs'));
   end

   dodisp = 1;
   if (nargs >= 8 && isequal(displayopt, 'off')), dodisp = 0; end
   ...
   if (size(g,1) ~= n) || (length(y) ~= n)
      error(message('stats:aoctool:InputSizeMismatch'));
   end
   if (ischar(g)), g = cellstr(g); end
   
   if ~isvector(g)
       error(message('stats:aoctool:GroupNotVector'));
   end

   nanrow = (isnan(x) | isnan(y));
   if (isnumeric(g))
      nanrow = (nanrow | isnan(g));
   elseif isa(g,'categorical')
      nanrow = (nanrow | isundefined(g));
   else
   ...
   if isa(g,'categorical')
       g = droplevels(g);  % toss out unused levels
   end
   [gi,gn] = grp2idx(g);
   ngroups = length(gn);
   n = length(x);
   ...
   if (var(x) == 0)
      error(message('stats:aoctool:BadX'));
   end
   if (ngroups < 2)
      error(message('stats:aoctool:NotEnoughGroups'));
   end
end
...
switch action
 ...
 case 'start',
  ...
  if nargs < 4 || isempty(alpha), alpha = 0.05; end

  if (nargs < 5),   xname = ''; end
  if isempty(xname), xname = inputname(1); end
  if isempty(xname), xname = getString(message('stats:aoctool:XValue')); end

  if (nargs < 6),   yname = ''; end
  if isempty(yname), yname = inputname(2); end
  if isempty(yname), yname = getString(message('stats:aoctool:YValue')); end

  if (nargs < 7),   gname = ''; end
  if isempty(gname), gname = inputname(3); end
  if isempty(gname), gname = getString(message('stats:aoctool:Group')); end
  ...
  if (nargs < 9)
     ...
     elseif ~(isequal(model,1)||isequal(model,2)||isequal(model,3) ...
             ...
             error(message('stats:aoctool:BadModel'));
     end
  end
  ...
  if (dodisp)
     ...
     set(aoc_fig,'PaperPositionMode','auto',...
                 'WindowButtonMotionFcn','aoctool(''motion'',0)',...
                 'WindowButtonDownFcn','aoctool(''down'')',...
                 'UserData',ud, 'Visible','on', ...
                 'HandleVisibility','callback');
     % Finished with AOCTOOL initialization.
     figure(aoc_fig);
     %if demo data was used, let user know how to use their own data.
     if ~isempty(noargmsg)
         warndlg(noargmsg, getString(message('stats:aoctool:AnalysisOfCovariance')),'modal');
     end
 ...
 case 'model',
  ...
  if ((~isempty(aov_fig)) || (~isempty(coef_fig)))
     [a0,c0] = maketabs(stats, gn, xname, gname, model, badbeta);
     updatedisplays(a0, c0, aoc_fig, aov_fig, coef_fig);
  end
  
 case 'conf',
  if (nargs > 1), conf = flag; end
  switch(conf)
  ...
  % Initialize group and model
  setgroup(newx, curgroup, model, stats, ud, y_field);
 ...
 case 'edittext',
  if isnan(newx) 
     ...
     set(x_field,'String',num2str(newx));
     warndlg(getString(message('stats:aoctool:MustBeNumber')), ...
             getString(message('stats:aoctool:AnalysisOfCovariance')),'modal');
     return;
  end
 ...
 case 'output',
  ...
  labels = {getString(message('stats:aoctool:LabelsParameters')),   ...
            getString(message('stats:aoctool:LabelsParameterCI')),  ...
            getString(message('stats:aoctool:LabelsPrediction')),   ...
            getString(message('stats:aoctool:LabelsPredictionCI')), ...
            getString(message('stats:aoctool:LabelsResiduals'))};
  ...
  export2wsdlg(labels, varnames, items, getString(message('stats:aoctool:ExportToWorkspace')));
...
tmp(end,4:5) = {[]};
atab = [{getString(message('stats:aoctool:ColHeadSource')) ...
         getString(message('stats:aoctool:ColHeadDf'))     ...
         getString(message('stats:aoctool:ColHeadSumSq'))  ...
         getString(message('stats:aoctool:ColHeadMeanSq')) ...
         getString(message('stats:aoctool:ColHeadF'))      ...
         getString(message('stats:aoctool:ColHeadProbGtF'))}; ...
        [{gname};{xname};{[gname '*' xname]};{getString(message('stats:aoctool:RowHeadError'))}] tmp];
...
if (model>2),            lab2 = {getString(message('stats:aoctool:RowHeadSlope'))}; end
if (model==5),           lab3 = tmp; end
rowlabels = [{getString(message('stats:aoctool:RowHeadIntercept'))};lab1;lab2;lab3];
...
tmp = num2cell(tmp);
ctab = [{getString(message('stats:aoctool:ColHeadTerm'))         ...
         getString(message('stats:aoctool:ColHeadEstimate'))     ...
         getString(message('stats:aoctool:ColHeadStdErr'))       ...
         getString(message('stats:aoctool:ColHeadT'))            ...
         getString(message('stats:aoctool:ColHeadProbGtAbsT'))}; ...
        rowlabels tmp];
...
set(y_field(1),'String',ytxt, 'Tag', 'YValue');
...
set(y_field(2),'String',ytxt, 'Tag', 'YValueBounds');
...
aocfig = figure('Units','normalized', 'Tag','aocfig', ...
       'Name',getString(message('stats:aoctool:ANOCOVAPredictionPlot')), 'Position',p, ...
       'NumberTitle','off', 'IntegerHandle','off');

% Brushing and linking don't work, so remove them to avoid confusion
delete(uigettool(aocfig,'Exploration.Brushing'))
delete(uigettool(aocfig,'DataManager.Linking'))
delete(findall(aocfig,'Tag','figDataManagerBrushTools'))
delete(findall(aocfig,'Tag','figBrush'))
delete(findall(aocfig,'Tag','figLinked'))
...
ppos = [.21 .18 .75 .72];
set(gca,'NextPlot','add','Position',ppos);
...
statbrushlink(plotHandles.fitline,false);
yrange = get(gca,'YLim');
xrange = get(gca,'XLim');
...
reference_line(1) = plot([newx newx],yrange,'k--','Tag','ReferenceLine1');
reference_line(2) = plot(xrange,[newy newy],'k:','Tag','ReferenceLine2');
...
uich.x_field = uicontrol('Style','edit', 'Units','normalized', ...
                         ...
                         'String',num2str(newx),'Tag','XValue');
...
uicontrol('Style','text','Units','normalized', ...
          ...
          'BackgroundColor',fcolor, 'String',getString(message('stats:aoctool:Model')));

uich.outputpopup=uicontrol('String', getString(message('stats:aoctool:ExportButton')),...
                           'Value',1, 'Units','normalized', ...
                           'Position',[0.01 0.13 0.17 0.05], ...
                           'CallBack','aoctool(''output'')',...
                           'Tag','ExportButton');

uich.grouppopup=uicontrol('Style','Popup', 'String', [{getString(message('stats:aoctool:AllGroups'))};gn], ...
                          'Value',curgroup+1, 'Units','normalized', ...
                          'Position',gfieldp, 'BackgroundColor','w', ...
                          'CallBack','aoctool(''group'')','Tag',getString(message('stats:aoctool:Group')));

modstring = getString(message('stats:aoctool:ModString'));
uich.modelpopup=uicontrol('Style','Popup', 'String',modstring, ...
                          'Value',model, 'Units','normalized', ...
                          'Position',mfieldp, 'BackgroundColor','w', ...
                          'CallBack','aoctool(''model'')','Tag','Model');

uicontrol('Style','Pushbutton','Units','normalized', ...
                         'Position',[0.01 0.07 0.17 0.05], ...
                         'Callback','close', 'String',getString(message('stats:aoctool:Close')),'Tag','CloseButton');
uicontrol('Style','Pushbutton','Units','normalized', ...
          'Position',[0.01 0.01 0.17 0.05], ...
          'Callback','aoctool(''closeall'')', 'String',getString(message('stats:aoctool:CloseAll')),...
          'Tag','CloseAllButton');

f = uimenu('Label',getString(message('stats:aoctool:MenuBounds')), 'Position', 4, 'UserData','conf', 'Tag', 'Bounds');
uimenu(f,'Label',getString(message('stats:aoctool:MenuSimultaneous')), ...
       'Callback','aoctool(''conf'',1)', 'UserData',1, 'Tag', 'Simultaneous');
uimenu(f,'Label',getString(message('stats:aoctool:MenuNonSimultaneous')), ...
       'Callback','aoctool(''conf'',2)', 'UserData',2, 'Tag', 'Nonsimultaneous');
uimenu(f,'Label',getString(message('stats:aoctool:MenuLine')), 'Separator','on', ...
       'Callback','aoctool(''conf'',3)', 'UserData',3, 'Tag', 'Line');
uimenu(f,'Label',getString(message('stats:aoctool:MenuObservation')), ...
       'Callback','aoctool(''conf'',4)', 'UserData',4, 'Tag', 'Observation');
uimenu(f,'Label',getString(message('stats:aoctool:MenuNone')), 'Separator','on', ...
       'Callback','aoctool(''conf'',5)', 'UserData',5 , 'Tag', 'None');
...
digits = [-1 -1 -1 -1 2 4];
if (nargin < 4)
   outfiga  = statdisptable(a0, getString(message('stats:aoctool:ANOCOVATestResults')), getString(message('stats:aoctool:ANOVATable')), ...
                            '', digits, [], parentfig, ...
...
if (nargin < 4)
   outfigc = statdisptable(c0, ...
                           getString(message('stats:aoctool:ANOCOVACoefficients')), getString(message('stats:aoctool:CoefficientEstimates')), ...
                           '', digits, [], parentfig, ...
...
newcov = E * bcov * (E');
if (anybad)
    newcov(2+ibad,2+ibad) = diag(Inf(length(ibad),1)); 
end
