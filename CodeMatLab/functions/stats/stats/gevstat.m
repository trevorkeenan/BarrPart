...
%   $Revision: 1.1.8.3 $  $Date: 2012/05/04 00:11:48 $

if nargin < 3
    error(message('stats:gevstat:TooFewInputs'));
end

[err,sizeOut] = statsizechk(3,k,sigma,mu);
if err > 0
    error(message('stats:gevstat:InputSizeMismatch'));
end
lngam1 = Inf(size(k),superiorfloat(k,sigma,mu)); % result for k >= 1
lngam2 = Inf(size(k),superiorfloat(k,sigma,mu)); % result for k >= 1/2
