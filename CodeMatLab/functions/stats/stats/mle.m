...
%   chosen from the following list of distributions:
%
%       'beta'                             Beta
%       'bernoulli'                        Bernoulli
%       'binomial'                         Binomial
...
%
%   See also BETAFIT, BINOFIT, EXPFIT, EVFIT, GAMFIT, LOGNFIT, NORMFIT,
...
%   $Revision: 1.1.8.5 $  $Date: 2012/03/27 18:32:19 $
...
if ischar(data)
    ...
    [dist,cens,freq,alpha,ntrials,options, ...
          pdf,logpdf,nloglf,~,otherArgs] = internal.stats.parseArgs(pnames, dflts, varargin{:}); %#ok<NASGU>
end
...
elseif ischar(dist)
    ...
    distNames = {'beta', 'bernoulli', 'binomial', 'exponential', 'extreme value', ...
                 ...
                 'geometric', 'lognormal', 'normal', 'negative binomial', ...
...
if ~isempty(ntrials) && ~strncmpi(dist, 'binomial', length(dist))
...
case 'beta'
    if ~isempty(cens)
        error(message('stats:mle:CensoringNotSupported', 'beta'));
    end
...
case {'gev' 'generalized extreme value'}
    if ~isempty(cens)
        error(message('stats:mle:CensoringNotSupported', 'generalized extreme value'));
    end
    if ~isempty(freq), data = expandInput(data,freq); end
    if nargout < 2
        phat = gevfit(data,[],options);
    else
        [phat,pci] = gevfit(data,alpha,options);
    end

case {'gp' 'generalized pareto'}
    if ~isempty(cens)
        error(message('stats:mle:CensoringNotSupported', 'generalized Pareto'));
    end
    if ~isempty(freq), data = expandInput(data,freq); end
    if nargout < 2
        phat = gpfit(data,[],options);
    else
        [phat,pci] = gpfit(data,alpha,options);
    end
...
case {'negative binomial', 'nbin'}
    if ~isempty(cens)
        error(message('stats:mle:CensoringNotSupported', 'negative binomial'));
    end
    if ~isempty(freq), data = expandInput(data,freq); end
    if nargout < 2
...
case 'poisson'
    if ~isempty(cens)
        error(message('stats:mle:CensoringNotSupported', 'Poisson'));
    end
    if ~isempty(freq), data = expandInput(data,freq); end
    if nargout < 2
        phat = poissfit(data);
...
case 'rayleigh'
    if ~isempty(cens)
        error(message('stats:mle:CensoringNotSupported', 'Rayleigh'));
    end
    if ~isempty(freq), data = expandInput(data,freq); end
    if nargout < 2
        phat = raylfit(data);
...
case {'weibull', 'wbl'}
    if ~strcmp(dist,'wbl')
        warning(message('stats:mle:ChangedParameters'));
    end
    if nargout < 2
...
otherwise
    ...
    elseif length(spec)>1
       error(message('stats:mle:AmbiguousDistName', name));
    end
    if nargout<2
