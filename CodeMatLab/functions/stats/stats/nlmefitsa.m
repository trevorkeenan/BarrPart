...
%   vector, a numeric vector, or a cell array of strings. See "help
%   groupingvariable" for more information.
%
...
if ~isvector(y) || ~isnumeric(y) || ~isreal(y)
    error(message('stats:nlmefitsa:BadY'));
elseif length(Id)~=NObs
    error(message('stats:nlmefitsa:BadGroupOrY'));
elseif ndims(X)>2 || ~isnumeric(X) || ~isreal(X)
    error(message('stats:nlmefitsa:BadX'));
elseif size(X,1)~=NObs
    error(message('stats:nlmefitsa:BadXOrY'));
end
[Id,sidx] = sort(Id);
X = X(sidx,:);
y = y(sidx);
...
IdMatches = bsxfun(@eq, Id, 1:NGroups);
...
if isempty(beta0)
    error(message('stats:nlmefitsa:MissingInitialValues'));
...
[errmod,y] = init_error_model(options.ErrorModel,y,options.ErrorParameters);
...
for jrep=1:nreps
    ...
    if nargout<3
        ...
        regFcn = @(b) computeyhat(structural_model,X,v,errmod,mean_phi,b,idxRandParams);
        % statjacobian needs each row to be a group, so create functions
        % that work with transposed b. Since this function does not support
        % observation-specific design matrices, the rowIdx input to
        % statjacobian is always Id.
        regTrFcn = @(bTr) computeyhat(structural_model,X,v,errmod,mean_phi,bTr',idxRandParams);
        jacFcn = @(f0, b) statjacobian(regTrFcn,b',options.DerivStep(idxRandParams),f0,Id);
        residuals = nlmeres(regFcn, jacFcn, y, IdMatches, Gj(idxRandOrder,idxRandOrder), errmod.type, statsj.errorparam, b_hat(:,uId,jrep));
        statsj.ires  = residuals.ires;
        statsj.pres  = residuals.pres;
        statsj.iwres = residuals.iwres;
        statsj.pwres = residuals.pwres;
        statsj.cwres = residuals.cwres;
    ...
    if stop && jrep<nreps % remove initialized but not filled-in values
        betas(:,jrep+1:end) = [];
        Gamma(:,:,jrep+1:end) = [];
        if nargout>=3 % If the user has asked for stats.
            stats(jrep+1:end) = [];
        end
        if nargout>=4 % If the user has asked for random effects.
            b_hat(:,:,jrep+1:end) = [];
        end
        break
    end
...
if strcmpi(id, 'stats:nlmeres:ErrorVarianceCloseToZero')
    warning(message('stats:nlmefitsa:ErrorVarianceCloseToZero'));
end
...
if ~isscalar(vect)
    ...
    error(message('stats:nlmefitsa:BadVectorization', okstring))
end
fvc = strcmpi(options.FunValCheck,'on');
if ~isa(modelfun,'function_handle')
    error(message('stats:nlmefitsa:InvalidModelFun'))
end
...
for kiter=1-options.NBurnIn:total_number_iterations_saem;
    ...
    if p>0
        disp(getString(message('stats:nlmefitsa:EstimateOfCorrelation')));
        disp(corrcov(Gamma_eta));
        error(message('stats:nlmefitsa:SingularCovariance'))
    end
...
if strcmpi(options.LogLikMethod,'is') 
    ...
    if ~isempty(mystream)
        oldstr = RandStream.setGlobalStream(mystream);
    end
    try
        st = saem_ll_is(st,GroupSizes,Id,y,X,v,errmod,fvc,vect,modelfun,transphi);
    catch me
        RandStream.setGlobalStream(oldstr);
        rethrow(me);
    end
    if ~isempty(mystream)
        RandStream.setGlobalStream(oldstr);
...
if ~isempty(outputFcn)
    if stop
        if nreps==1
            warning(message('stats:nlmefitsa:AlgorithmTerminatedByOutputFcn'));
        else
            warning(message('stats:nlmefitsa:ReplicateTerminatedByOutputFcn', jrep, nreps));
        end
    end
...
if verbose
    fprintf('%s',getString(message('stats:nlmefitsa:NumberOfIterations',total_number_iterations_saem)));
    fprintf('%s',getString(message('stats:nlmefitsa:RootMeanSquareError',errmod2text(errmod))));
    fprintf(getString(message('stats:nlmefitsa:EstimatedCoefficients')));
    fprintf('   %g',betas);
    fprintf('\n');
end
end
...
elseif (isnumeric(v)||islogical(v)) && isequal(size(v),[number_etas,number_etas])
    ...
    error(message('stats:nlmefitsa:BadCovPattern'))
end
...
elseif strcmpi(options.OptimFun,'fminunc')
    if ~license('test', 'Optimization_Toolbox') || ~exist('fminunc', 'file')
        error(message('stats:nlmefitsa:RequiresOptimToolbox'));
    end
    canCheckout = license('checkout', 'Optimization_Toolbox');
    if ~canCheckout
        error(message('stats:nlmefitsa:RequiresOptimLicense'));
    end        

    opt = optimset('Display','off','LargeScale','off');
    options.OptimFun = @(f,x) fminunc(f,x,opt);
else
    error(message('stats:nlmefitsa:BadOptimFun'));
end

val = options.NBurnIn;
if ~(isnumeric(val) && isscalar(val) && val>=0 && val==round(val))
    error(message('stats:nlmefitsa:BadNBurnIn'));
end
...
elseif ~(isnumeric(val) && isscalar(val) && val>=1 && val==round(val))
    error(message('stats:nlmefitsa:BadNChains'));
end
...
elseif ~ (isnumeric(val) && isvector(val) && numel(val)==3 && ...
    ...
    error(message('stats:nlmefitsa:BadNIterations'));
end
...
elseif ~ (isnumeric(val) && isvector(val) && numel(val)==3 && sum(val)>0 ...
    ...
    error(message('stats:nlmefitsa:BadNMCMCIterations'));
end

val = options.ComputeStdErrors;
if ~(isscalar(val) && (val==0 || val==1))
    error(message('stats:nlmefitsa:BadComputeStdErrors'))
end
...
if ~any(j)
    okstring = [sprintf('''%s'', ', ok{1:end-1}) 'and ''' ok{end} ''''];
    error(message('stats:nlmefitsa:BadLogLikMethod', okstring));
end
...
elseif ~(isnumeric(val) && isscalar(val) && val>=1 && val==round(val))
    error(message('stats:nlmefitsa:BadReplicates'));
elseif nstarts>1 && options.Replicates~=nstarts
    error(message('stats:nlmefitsa:ReplicatesConflict'));
end

val = options.DerivStep;
if isscalar(val)
    % Expand to val to be a row vector of the number of parameters,
    % which is equal to the number of rows in A.
    val = repmat(val, 1, number_parameters);
end
% Ensure val is a row vector.
val = val(:)';
options.DerivStep = val;
end
...
if ~isempty(Gamma_ini) && ~(isnumeric(Gamma_ini) && ...
    ...
    error(message('stats:nlmefitsa:BadCovariance', nrandparams, nrandparams));
end
...
if ~isempty(Gamma_ini)
    ...
    [~,p] = chol(Gamma_ini);
    if p~=0
       error(message('stats:nlmefitsa:CovarianceNotPosDef'));
    end
...
if isempty(options.Streams)
    ...
    elseif isscalar(options.Streams)
        ...
        error(message('stats:nlmefitsa:BadStream'))
    end
...
if ~isvector(paramtransform) || numel(paramtransform)~=number_parameters ...
    ...
    error(message('stats:nlmefitsa:BadParamTransform', number_parameters));
end
if nargin<3 || ~deriv
...
if funvalcheck && any(~isfinite(f))
    error(message('stats:nlmefitsa:CheckFunVals'));
end
...
statsetDefaults = statset('nlmefitsa');
options.DerivStep = statsetDefaults.DerivStep;
options.FunValCheck = statsetDefaults.FunValCheck;
options.Streams = statsetDefaults.Streams;
options.OutputFcn = statsetDefaults.OutputFcn; % output function or functions
options.verbose = 0;

numvaragin = numel(varargin);
if numvaragin > 0
    if rem(numvaragin, 2)
        error(message('stats:nlmefitsa:IncorrectNumberOfArguments'));
    end
    ...
    for j = 1:2:numvaragin
        ...
        if isempty(property_name) || ~ischar(property_name) || size(property_name,1)>1
            error(message('stats:nlmefitsa:BadParameterName'));
        end
        ...
        if isempty(property_index)
            error(message('stats:nlmefitsa:UnknownParameterName', property_name));
        elseif length(property_index) > 1
            error(message('stats:nlmefitsa:AmbiguousParameterName', property_name));
        elseif property_index>1
            ...
            if ~isstruct(property_value)
                error(message('stats:nlmefitsa:BadOptions'));
            end
            
            try
                newopt = statset(statsetDefaults, property_value);
            catch ME
                m = message('stats:nlmefitsa:BadOptionsSetting');
                throw(addCause(MException(m.Identifier,'%s',getString(m)),ME));
            end
            
            options.DerivStep = setifnotempty(newopt.DerivStep,options.DerivStep);
...
if 1 < ~isempty(options.FEParamsSelect) + ~isempty(options.FEConstDesign) + ...
    ...
    error(message('stats:nlmefitsa:MultipleFixedDesigns'))
end
if 1 < ~isempty(options.REParamsSelect) + ~isempty(options.REConstDesign) + ...
    ...
    error(message('stats:nlmefitsa:MultipleRandomDesigns'))
end
...
if any(numparams>1)
    error(message('stats:nlmefitsa:RepeatedCoefficients'));
end
...
if ~isempty(ObsDesign)
    ...
    if ndims(A)~=3 || size(A,3)~=nobs
        error(message('stats:nlmefitsa:BadDesign3D', fixedorrand, sprintf( '%d', nobs )));
    elseif (size(A,2)~=nb && ~israndom)
        error(message('stats:nlmefitsa:BadFixedDesign', nb));
    end
    ...
    for j=2:length(grp)
        if grp(j)==grp(j-1) && ~isequal(A(:,:,j),A(:,:,j-1))
            error(message('stats:nlmefitsa:NoObs'))
        end
    end
    k = [1, 1+find(diff(grp))'];
    GroupDesign = A(:,:,k);
...
if ~isempty(GroupDesign)
    ...
    if ndims(A)~=3 || size(A,3)~=ngrps
        error(message('stats:nlmefitsa:BadDesign3D', fixedorrand, sprintf( '%d', ngrps )));
    elseif (size(A,2)~=nb && ~israndom)
        error(message('stats:nlmefitsa:BadFixedDesign', nb));
    end
    designtype = 2;
    ...
    if israndom
        ...
        if ~isequal(A,repmat(A(:,:,1),[1,1,ngrps]))
            if isempty(ObsDesign)
                error(message('stats:nlmefitsa:NoGroup'));
            else
                error(message('stats:nlmefitsa:NoObs'))
            end
        end
...
if ~isempty(ConstDesign)
    ...
    if ndims(A)~=2 ...
        ...
        error(message('stats:nlmefitsa:BadDesign', fixedorrand));
    end
    designtype = 3;
...
elseif ~isempty(ParamsSelect)
    ...
    if islogical(v)
        if israndom && length(v)~=nb
            error(message('stats:nlmefitsa:BadREParamsSelect', nb));
        end
        v = find(v);
    end
    ...
    if israndom && isempty(v)
        error(message('stats:nlmefitsa:NoRandomEffects'));
    elseif any(diff(sort(v))==0)
        error(message('stats:nlmefitsa:RepeatedParamsSelect'));
    end
    A = accumarray([v(:), (1:length(v))'], 1);
    if israndom && size(A,1)<nb
        A(nb,1) = 0;
    elseif ~israndom && size(A,2)~=nb
        error(message('stats:nlmefitsa:BadFEParamsSelect'));
    end
    PageA = ones(nobs,1);
    designtype = 4;
end
...
if ~all(B(:)==0 | B(:)==1)
    error(message('stats:nlmefitsa:RandomSlope'));
end

% --- 3. At most one random effect per parameter
numrandom = sum(B,2);
if any(numrandom>1)
    error(message('stats:nlmefitsa:MultipleRandom'));
end

% --- 4. At most one parameter per random effect
numparams = sum(B,1);
if any(numparams>1)
    error(message('stats:nlmefitsa:MultipleParameters'));
end
      
% --- 5. Cannot have random effects that are not assigned to parameters
if any(numparams==0)
    error(message('stats:nlmefitsa:UnusedRandomEffects'));
end
end
...
switch nnodes_gq
    ...
    otherwise
        disp(getString(message('stats:nlmefitsa:NumberOfPointsShouldNotBeGreaterThan25')))
        nodes=[];
        weights=[];
        return
end
...
while ~isempty(itest_phi)
    ...
    if kt == 100 % TODOpax add a parameter for this constant.        
        error(message('stats:nlmefitsa:FailedInitialParameterGuess'));
    end
...
elseif strcmp(errmod.type,'proportional')
    ...
    ab = abs(fminsearch(@(x) error_ab(x,y,f),[a b]));
    if  ~dampflag
...
if ~isscalar(i)
    okstring = [sprintf('''%s'', ', ok{1:end-1}) 'and ''' ok{end} ''''];
    if isempty(i)
        error(message('stats:nlmefitsa:BadErrorModel', okstring));
    else
        error(message('stats:nlmefitsa:AmbiguousErrorModel', okstring));
    end
end
error_model = ok{i};
...
if ~isempty(params0)
    if numel(params0)<1 || numel(params0)>2 || ~isnumeric(params0) || all(params0<=0)
        error(message('stats:nlmefitsa:BadErrorParam'))
    end
    switch error_model
        case 'combined'
            if numel(params0)~=2
                error(message('stats:nlmefitsa:BadCombinedParam', error_model))
            end
        case 'proportional'
            % Only b is needed; a must equal 0 if specified.
            if numel(params0)==2 && params0(1)~=0
                error(message('stats:nlmefitsa:BadErrorParam1', error_model))
            end
        case {'constant', 'exponential'}
            % Only a is needed; b must equal 0 if specified.
            if numel(params0)==2 && params0(2)~=0
                error(message('stats:nlmefitsa:BadErrorParam2', error_model))
            end
    end
end
...
switch error_model
    ...
    case 'combined' % y = f + (a+b*f)*e
        ...
        p = [a b];
        indices = 1:2;
    case 'exponential' % y = f*exp(a*e), or log(y) = log(f) + a*e
        if isempty(params0)
            a = 1;
        else
            a = params0(1);
