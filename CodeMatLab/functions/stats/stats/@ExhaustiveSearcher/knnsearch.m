...
%   $Revision: 1.1.8.6 $  $Date: 2012/01/06 23:01:58 $
...
pnames = { 'k'  'distance'  'p', 'cov', 'scale','includeties'};
dflts =  { 1    []           []   []     []    false };
[numNN,distMetric, minExp, mahaCov, seucInvWgt, includeTies] = ...
...
if ~logical(includeTies)
    ...
    [dist2, idx2]=pdist2(obj.X,Y, distMetric, arg{:}, 'smallest',K);
        ...
        for i=1:numel(notDoneIdx)
            ...
            if ~isempty(tempT)
                ...
                [dTemp, iTemp] = pdist2(obj.X,Y(notDoneIdx(i),:),...
                    distMetric, arg{:}, 'radius',dist2(i,numNN)+eps);
                dist(tempIdx) = dTemp;
                idx(tempIdx) = iTemp;
