...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:26:09 $

if nargin <  3,
    error(message('stats:nctinv:TooFewInputs'));
end

[errorcode, p, nu, delta] = distchck(3,p,nu,delta);

if errorcode > 0
    error(message('stats:nctinv:InputSizeMismatch'));
end
...
if count==count_limit
    warning(message('stats:nctinv:NoConvergence', sprintf( '%g', max( abs( h( : ) ) ) )));
end
