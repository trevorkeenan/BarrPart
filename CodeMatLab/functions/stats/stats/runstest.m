...
%   value V. The default value of V is the mean of X.
%
...
%   $Revision: 1.1.8.4 $  $Date: 2011/06/06 17:19:16 $

% Get required data input vector and remove NaNs
narginchk(1,Inf);
if ~isvector(x) && ~isempty(x)
    error(message('stats:runstest:VectorRequired'));
end
x(isnan(x)) = [];
...
elseif isequal(lower(v),'ud')
    ...
    error(message('stats:runstest:BadV'));
end
...
elseif ~isscalar(alpha) || alpha <= 0 || alpha >= 1
    error(message('stats:runstest:BadAlpha'));
end
if isempty(tail)
    tail = 0;   % both
...
if ~isscalar(tail) || ~isnumeric(tail) || ~ismember(tail,-1:1)
    error(message('stats:runstest:BadTail'));
end
if ~isempty(method)
...
if updown
   ...
   if any(x==0)
       x(x==0) = [];
       N = numel(x) + 1;
       warning(message('stats:runstest:RepeatedValuesOmitted'));
   end
   x = double(x>0);
else
   if any(x==v)
       x(x==v) = [];
       N = numel(x);
       warning(message('stats:runstest:ValuesOmitted'));
   end
   x = double(x(:) > v);
end
...
elseif method(1)=='e' && updown && N>50
    warning(message('stats:runstest:NoExactMethod'));
    doexact = false;
else
    doexact = method(1)=='e';
end
