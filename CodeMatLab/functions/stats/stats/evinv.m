...
%   $Revision: 1.1.6.3 $  $Date: 2011/05/09 01:24:52 $

if nargin<1
    error(message('stats:evinv:TooFewInputs'));
end
if nargin < 2
    mu = 0;
end
...
if nargout>2
   if nargin<4
      error(message('stats:evinv:MissedCov'));
   end
   if ~isequal(size(pcov),[2 2])
      error(message('stats:evinv:BadShapedCovariance'));
   end
   if nargin<5
      alpha = 0.05;
   elseif ~isnumeric(alpha) || numel(alpha)~=1 || alpha<=0 || alpha>=1
      error(message('stats:evinv:BadAlpha'));
   end
end
...
try
    ...
    error(message('stats:evinv:InputSizeMismatch'));
end
...
if nargout>=2
   ...
   if any(xvar<0)
      error(message('stats:evinv:BadCovariance'));
   end
   z = -norminv(alpha/2);
