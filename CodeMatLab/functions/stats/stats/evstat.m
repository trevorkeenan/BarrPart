...
%   mirror image of this distribution can be used to model maxima.  If Y has a
%   Weibull distribution, then X=log(Y) has the type 1 extreme value
%   distribution.
%
...
%   $Revision: 1.1.8.2 $  $Date: 2010/10/08 17:23:21 $

if nargin < 2
    error(message('stats:evstat:TooFewInputs'));
end
...
try
    ...
    error(message('stats:evstat:InputSizeMismatch'));
end
