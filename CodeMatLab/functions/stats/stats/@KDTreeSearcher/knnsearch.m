...
%   $Revision: 1.1.8.7 $ $Date: 2011/12/11 08:05:33 $
...
pnames = { 'k'  'distance', 'p','includeties'  };
dflts =  { 1    []          [] , false };
[numNN,distMetric, minExp,includeTies] = ...
...
if includeTies && any(wasNaNY)
    ...
    idxTemp = repmat(cast(1:numNN, outClass), lenNaNY,1);
...
if numDiff > 0 %the number of X points without NaN is smaller than numNN
    ...
    if ~includeTies %no need to check ties
        ...
        if any(wasNaNY)
            idx(wasNaNY,numNN2+1:numNN) = repmat(cast(numNN2+1:numNN,outClass),sum(wasNaNY),1);
        end
        if nargout > 1
            dist = [dist nan(nY,numDiff,outClass)];
        end
        ...
        if nargout < 2
            for i=1:nY
                if ~wasNaNY(i)
                    idx{i}=[idx{i} cast(nanIdx, outClass)];
                 end
            end
        else
            for i=1:nY
                if ~wasNaNY(i)
                    idx{i}=[idx{i} cast(nanIdx, outClass)];
                    dist{i}= [dist{i}, cast(nan(1,numDiff), outClass)];
                end
