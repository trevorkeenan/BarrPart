...
%
%   XPTREAD only supports single data sets per file. XPTREAD does not
...
%   $Revision: 1.1.8.7 $  $Date: 2011/05/09 01:27:21 $
...
pnames = {'readobsnames'};
dflts =  { false };
readObsFlag = internal.stats.parseArgs(pnames, dflts, varargin{:});
...
[fid,theMessage] = fopen(filename,'r','b');
if fid == -1
    error(message('stats:xptread:CouldNotOpenFile', origFilename, theMessage));
end
...
elseif ~isempty(strfind(line,'COMPRESSED'))
    m = message('stats:xptread:CompressedFile');
...
for theObs = 1:numObs
    ...
    for theVar = 1:numVariables
        if var(theVar).ntype == 1
            ...
            if numel(checkVal)>1 && all(checkVal(2:end)==0) && ...
                    (   (checkVal(1)>='A'&&checkVal(1)<='Z') ...
                      || checkVal(1)=='_' ...
                      || double(checkVal(1))== '.')
                ...
                theVal = ibm2ieee(double(checkVal'));
                 ...
                 % permitted but no examples of them were found. This code
                 % should unpack these.
                 %
...
if readObsFlag
    ...
    try
        ...
        if strcmpi(theEx.identifier,'stats:dataset:setobsnames:DuplicateObsnames')
            warning(message('stats:xptread:DuplicateObsnames'));
        elseif strcmpi(theEx.identifier,'stats:dataset:setobsnames:InvalidObsnames')
            warning(message('stats:xptread:InvalidObsnames'));
        else
            rethrow(theEx)
        end
    end
...
m = message('stats:xptread:BadXPTFormat');
throwAsCaller(MException(m.Identifier,'%s',getString(m)));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
