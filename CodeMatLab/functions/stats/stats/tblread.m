...
% $Revision: 1.1.8.4 $  $Date: 2011/09/08 23:39:23 $
...
if nargin < 2  
   ...
   otherwise
      ...
      warning(message('stats:tblread:DelimiterNotSupported',delimiter));      
   end
end
...
if fid == -1
   disp(getString(message('stats:tblread:UnableToOpenFile')));
   return
end

lf = sprintf('\n'); % line feed is platform dependent
...
if length(delimitidx) ~= nobs*nvars
   if length(delimitidx) == nobs*(nvars-1)
      ...
      error(message('stats:tblread:BadFileFormat'));
   end
end
if nvars > 1
