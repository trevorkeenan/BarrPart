...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:27:17 $

narginchk(2,4);
if ~isvector(data)
    error(message('stats:wbllike:InvalidData'));
end
...
elseif ~isequal(size(data), size(censoring))
    error(message('stats:wbllike:CensSizeMismatch'));
end
...
elseif isequal(size(data), size(freq))
    ...
    error(message('stats:wbllike:FreqSizeMismatch'));
end
