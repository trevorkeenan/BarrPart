...
% $Revision: 1.1.8.5 $  $Date: 2011/10/15 02:06:12 $

if numel(specs) ~= 2,
   error(message('stats:capaplot:SpecsBadSize'));
end
...
if lb == -Inf && ub == Inf
   error(message('stats:capaplot:BadSpecs'));
end

if ~isvector(data)
   error(message('stats:capaplot:VectorRequired'));
end
...
% str = sprintf('Probability Between Limits = %s',num2str(p));
title(getString(message('stats:capaplot:ProbabilityBetweenLimits',num2str(p))));
% title(str);
if ~isempty(np) 
   set(gca,'NextPlot',np);
end
