...
%   $Revision: 1.1.8.2 $    $Date: 2010/10/08 17:28:58 $
...
%          Journal of Complexity, Vol. 14, pp. 527-556, 1998.
    ...
    methods
        ...
        if ~any(strcmpi(val, {'standard', 'graycode'}))
            error(message('stats:sobolset:InvalidPointOrder'));
        end
    ...
    methods(Access = 'protected')
        ...
        if ~isempty(ExistingS)
            error(message('stats:sobolset:InvalidScramble'));
        end
        ...
        if ~isempty(S) && ~isempty(S.Options)
            warning(message('stats:sobolset:IncorrectScrambleOptions'));
        end
        end
    end
