...
%   $Revision: 1.1.8.6 $    $Date: 2012/02/02 19:09:46 $
    ...
    methods
        ...
        if nargin
            ...
            if mod(length(varargin),2)~=0
                error(message('stats:qrandset:WrongNumberArgs'));
            end
            for n=1:2:length(varargin)
                if ~ischar(varargin{n})
                    error(message('stats:qrandset:BadPropertyName'));
                end
                ...
                elseif NMatch==0
                    % No match at all
                error(message('stats:qrandset:NoSuchField', varargin{ n }, 'qrandset'));
                else
                    % Ambiguous match
                    error(message('stats:qrandset:AmbiguousProperty', class( obj ), varargin{ n }));
                end
            end
        end
    ...
    methods
        ...
        if ~checkRealScalarInt(val) || val<1 || val>obj.getMaxDims
            if isfinite(obj.getMaxDims)
                error(message('stats:qrandset:InvalidNumberOfDimensions', obj.getMaxDims));
            else
                error(message('stats:qrandset:InvalidNumberOfDimensionsInf'));
            end
        end
        ...
        if ~checkRealScalarInt(val) || val<0
            error(message('stats:qrandset:InvalidSkip'));
        end
        ...
        if obj.NumPoints<1
            error(message('stats:qrandset:SkipTooLarge'));
        end
        end

        function obj = set.Leap(obj, val)
        if ~checkRealScalarInt(val) || val<0
            error(message('stats:qrandset:InvalidLeap'));
        end
        obj.Leap = double(val);
        obj = alterSkipLeap(obj);
        obj = recalcNumPoints(obj);
        ...
        if ~isempty(S) && ~isScrambleStruct(S)
            error(message('stats:qrandset:InvalidScrambleMethod'));
        end
    ...
    methods
        ...
        fprintf('%s\n',getString(message('stats:qrandset:DispPointSet', ...
            obj.Type, obj.Dimensions, num2str(obj.NumPoints))));
        fprintf(LooseLine);
        fprintf('%s:\n',getString(message('stats:qrandset:DispProperties')));
        dispProperties(obj)
        fprintf(LooseLine);
        end
        ...
        if nargin==1
            ...
            if ~checkRangeScalarInt(dim, 1, inf)
                error(message('stats:QRPointSet:InvalidDimension'));
            end
            if dim==1
    ...
    methods
        ...
        if ischar(Action)
            ...
            error(message('stats:qrandset:InvalidArgument'));
        end
        end
    end
    ...
    methods(Access = 'protected')
        ...
        if ~isempty(S)
            ...
            if ~isempty(S.Type)
                ...
                Matched = find(strncmpi(S.Type, ValidNames, length(S.Type)));
            end
            if length(Matched)>1
                error(message('stats:qrandset:AmbiguousScramble', S.Type));
            elseif isempty(Matched)
                error(message('stats:qrandset:UnknownScramble', S.Type));
            end
    ...
    methods
        ...
        if ~checkRangeScalarInt(NetSize, 0, obj.NumPoints)    
            error(message('stats:qrandset:InvalidNetSize'));
        end
    ...
    methods
        ...
        if strcmp(S(1).type, '.')
            ...
            if isPublic(obj, Identifier, 'get') || isPublic(obj, Identifier, 'call')
                ...
                error(message('stats:qrandset:NoSuchMethodOrField', Identifier, class( obj )));
            end
        ...
        elseif strcmp(S(1).type, '()')
            ...
            error(message('stats:qrandset:CellRefFromNonCell'));
        end
        end
    end
    ...
    methods(Hidden)
        ...
        if strcmp(S(1).type, '.')
            ...
            elseif isPublic(obj, Identifier, 'get')
                ...
                % it doesn't exist
                error(message('stats:qrandset:SetProhibited', Identifier, class( obj )));
            else
                error(message('stats:qrandset:NoSuchField', Identifier, class( obj )));
            end

        elseif strcmp(S(1).type, '()')
            error(message('stats:qrandset:AssignmentNotAllowed', class( obj )));
        else % {} assignment
            error(message('stats:qrandset:CellAssToNonCell'));
        end
        end
        ...
        function varargout = transpose(varargin)
            error(message('stats:qrandset:TransposeNotAllowed'));
        end
        function varargout = ctranspose(varargin)
            error(message('stats:qrandset:TransposeNotAllowed'));
        end
        function varargout = reshape(varargin)
            error(message('stats:qrandset:ReshapeNotAllowed'));
        end
        function varargout = permute(varargin)
            error(message('stats:qrandset:PermuteNotAllowed'));
        end 
    end
    methods(Hidden,Static)
        function obj = empty(varargin)
            error(message('stats:qrandset:EmptyNotAllowed'));
        end
    end
end
...
MatchIdx = find(strncmpi(Ident,CorrectIdents,length(Ident)));
...
m = message('stats:qrandset:NoCatAllowed');
...
if length(IdxArgs)==2 ...
    ...
    elseif ~checkRangeInts(PtIdx, 1, obj.NumPoints)
        error(message('stats:qrandset:InvalidPointIndices'));
    end
    ...
    elseif ~islogical(Cols) && ~checkRangeInts(Cols, 1, obj.Dimensions)
        error(message('stats:qrandset:InvalidDimensionIndices'));
    end
...
elseif length(IdxArgs)==1
    ...
    elseif ~checkRangeInts(PtIdx, 1, (obj.NumPoints*obj.Dimensions))
        error(message('stats:qrandset:InvalidIndices'));
    end
    ...
    error(message('stats:qrandset:badsubscript'));
end
