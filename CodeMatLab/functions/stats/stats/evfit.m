...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:24:51 $

% Illegal data return an error.
if ~isvector(x)
    error(message('stats:evfit:VectorRequired'));
end
...
elseif ~isempty(censoring) && ~isequal(size(x), size(censoring))
    error(message('stats:evfit:XCensSizeMismatch'));
end
if nargin < 4 || isempty(freq)
    freq = ones(size(x));
elseif ~isempty(freq) && ~isequal(size(x), size(freq))
    error(message('stats:evfit:XFreqSizeMismatch'));
else
...
if lkeqn(sigmahat, x0, freq, wgtmeanUnc) > 0
    ...
    while lkeqn(lower, x0, freq, wgtmeanUnc) > 0
        ...
        if lower < realmin(classX) % underflow, no positive root
            error(message('stats:evfit:NoSolution'));
        end
    end
else
...
elseif eps(lkeqnval) > options.TolX
    warning(message('stats:evfit:IllConditioned'));
end
...
if nargout == 2
    ...
    [~, acov] = evlike(parmhat, x, censoring, freq);
