...
%   $Revision: 1.1.8.7 $  $Date: 2011/08/29 20:38:19 $
...
if isempty(X)
    error(message('stats:controlchart:BadX'));
end
...
elseif isnumeric(X) && ndims(X)==2
    ...
    error(message('stats:controlchart:BadData'));
end
if ~isempty(G) && (numel(G) ~= size(X,1))
     error(message('stats:controlchart:InputSizeMismatch'))
end
...
defaults = {'xbar'       ''      0.4      []       []       []      [] ...
            3            'std'   []       []       5        'on'};
[charttype  label lambda limits parent rules meanval ...
...
if ~isnumeric(sigma)
    sigma = internal.stats.getParamVal(sigma, {'range' 'std' 'variance'}, 'SIGMA');
end

if ~(isscalar(dodisp) && islogical(dodisp))
    dodisp = internal.stats.getParamVal(dodisp, {'on' 'off'}, 'DISPLAY');
...
if grouped && any(ismember({'ma' 'mr'},cctypes))
    error(message('stats:controlchart:NoSubgroups'));
end
...
% Fix that before continuing, and store both versions in case they are
% needed for a mixture of plot types.
nSigma = stats.n;   % n for sigma estimate and for moving/grouped plots
if ismember('i',cctypes) || (~grouped && ismember('ewma',cctypes))
    nIndividual = ones(size(stats.n));
    nIndividual(isnan(stats.i)) = 0;
    stats.n = nIndividual;  % n for I and ungrouped EWMA plots
end
...
for jplot=1:nplots
    ...
    if isequal(cctype,'i') || (~grouped && isequal(cctype,'ewma'))
        plotN = nIndividual;
    else
        plotN = nSigma;
    end
    [pts,cl,lcl,ucl,se,ngrp,ooc] = getplotdata(stats,cctype,nsigma,...
                                  lambda,limits,~isempty(rules),plotN);
    ...
    if jplot==1
        displaynames = {getString(message('stats:controlchart:LegendData'))      ...
                        getString(message('stats:controlchart:LegendViolation')) ...
                        getString(message('stats:controlchart:LegendCenter'))    ...
                        getString(message('stats:controlchart:LegendLCLUCL'))    ...
                        getString(message('stats:controlchart:LegendSpecs'))};
        legend(h,displaynames{1:length(h)},'Location','NorthEastOutside');
        if nplots>1
            title(ax,getString(message('stats:controlchart:TitleControlCharts')));
        else
            title(ax,getString(message('stats:controlchart:TitleControlChart',upper(cctype))));
        end
    end
end
...
if ~isempty(nestedstr.rv{nestedj}) % create appropriate cursor text
    txt = getString(message('stats:controlchart:CursorOutOfControl',...
        nestedj,nestedlbl,nestedstr.rv{nestedj}));
else
    txt = getString(message('stats:controlchart:CursorInControl',nestedj,nestedlbl));
end
end
...
if ~grouped && any(ccgroupnum==1)
   error(message('stats:controlchart:GroupRequired'));
elseif length(ccgroupnum)~=1
    error(message('stats:controlchart:BadChartType'));
end
...
t = toolow & haverv;
if any(t)
    str.rv(t) = strcat(getString(message('stats:controlchart:CursorLtLCL')), ',', ruletxt(t));
end
t = toolow & ~haverv;
if any(t)
    str.rv(t) = {getString(message('stats:controlchart:CursorLtLCL'))};
end
t = toohigh & haverv;
if any(t)
    str.rv(t) = strcat(getString(message('stats:controlchart:CursorGtUCL')), ',', ruletxt(t));
end
t = toohigh & ~haverv;
if any(t)
    str.rv(t) = {getString(message('stats:controlchart:CursorGtUCL'))};
end
...
if ~isempty(label)
    if ~(ischar(label) && size(label,1)==ngroups) && ...
       ~(iscellstr(label) && numel(label)==ngroups)
        error(message('stats:controlchart:BadLabel', ngroups));
    end
end

if ~isempty(lambda) && ...
   ~(isnumeric(lambda) && isscalar(lambda) && lambda>0 && lambda<1)
    error(message('stats:controlchart:BadLambda'));
end

if ~isempty(limits) && ~(isnumeric(limits) && numel(limits)==3 && ...
                         limits(2)>limits(1) && limits(3)>limits(2))
    error(message('stats:controlchart:BadLimits'))
end

if ~isempty(parent)
    if ~(isscalar(parent) && ishghandle(parent) && ...
         isequal(get(parent,'type'),'axes'))
        error(message('stats:controlchart:BadParent'));
    end
    if length(cctypes)>1
        error(message('stats:controlchart:ParentNotAllowed'));
    end
end

if ~(isnumeric(nsigma) && isscalar(nsigma) && nsigma>0)
    error(message('stats:controlchart:BadNSigma'));
end

if ~isempty(meanval)
    if ~(isnumeric(meanval) && isscalar(meanval))
        error(message('stats:controlchart:BadMean'));
    elseif any(ismember({'p' 'np'},cctypes)) && (meanval<=0 || meanval>=1) 
        error(message('stats:controlchart:BadMeanForPNP'));
    elseif any(ismember({'c' 'u'},cctypes)) && meanval<=0
        error(message('stats:controlchart:BadMeanForCU'));
    end
end
...
if ~(isequal(sigma,'std')      || isequal(sigma,'range') || ...
    ...
    error(message('stats:controlchart:BadSigma'));
end

if ~isempty(specs) && ~(isnumeric(specs) && isvector(specs))
    error(message('stats:controlchart:BadSpecs'))
end

if ~isempty(unit) && (~isnumeric(unit) || any(unit(:)<=0) || ...
                      ~(isscalar(unit) || isequal(size(unit),size(X))))
    error(message('stats:controlchart:BadUnit'));
end
if isempty(unit) && any(ismember({'p' 'np'},cctypes))
    error(message('stats:controlchart:UnitMissingForPNP'));
end

if ~isnumeric(width) || ~isscalar(width) || width<=0 || width~=round(width)
    error(message('stats:controlchart:BadWidth'))
elseif width>size(X,1)
    error(message('stats:controlchart:InvalidWidth'))
end
end
