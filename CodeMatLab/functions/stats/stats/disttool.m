...
%   $Revision: 1.1.8.7 $  $Date: 2012/05/08 20:43:02 $
...
if ~strcmp(action,'start')   
   if isempty(gcbf)
      ...
      error(message('stats:disttool:BadInput'));
   end
...
if strcmp(action,'start'),
   ...
   yvalues = cdf(ud.dists(dfltDist).code,xvalues,pval{:});
   newy = cdf(ud.dists(dfltDist).code,newx,pval{:});

   %   Create Cumulative Distribution Plot
   ud.dist_fig = figure('Tag','distfig',...
                        'NumberTitle','off','IntegerHandle','off',...
                        'Name',getString(message('stats:disttool:TitleDisttool')));
...
% Define graphics objects
   for idx = 1:3
       ...
       ud.lowerboundtext(idx) = uicontrol('Style','text','Units','normalized',...
         ...
         'BackgroundColor',figcolor,'String', getString(message('stats:disttool:LabelLowerBound')),...
       ...
       ud.upperboundtext(idx) = uicontrol('Style','text','Units','normalized',...
         ...
         'BackgroundColor',figcolor,'String', getString(message('stats:disttool:LabelUpperBound')),...
   ...
   ud.yaxistext=uicontrol('Style','Text','Units','normalized',...
       ...
       'ForegroundColor','k','BackgroundColor',figcolor,...
       'String',getString(message('stats:disttool:LabelProbability')),'Tag','yaxistext'); 
        
   ud.distribution=uicontrol('Style','Text','String',getString(message('stats:disttool:LabelDistribution')),...
       'ForegroundColor','k','BackgroundColor',figcolor,...
   ...
   ud.type=uicontrol('Style', 'Text', 'String', getString(message('stats:disttool:LabelFunctionType')), ...
                     'ForegroundColor','k','BackgroundColor',figcolor,...
   ...
   ud.functionpopup=uicontrol('Style','Popup','String',...
        {getString(message('stats:disttool:CDF')) getString(message('stats:disttool:PDF'))},'Value',1,'Units','normalized',...
        'Position',[xpos .92 .15 .06],'BackgroundColor','w',...
        'Callback','disttool(''changefunction'')','Tag','functionpopup');
        
   ud.Xlabel=uicontrol('Style', 'Text', 'String', getString(message('stats:disttool:LabelX')), ...
                             'ForegroundColor','k','BackgroundColor',figcolor,...
...
if iscdf
   newy = cdf(code,newx,pval{:});
else
   newy = pdf(code,newx,pval{:});
end
...
if isnan(newx)
    ...
    warndlg(getString(message('stats:disttool:WarnDlgCritValNum')));
    return;
end
xrange = get(gcba,'XLim');
if newx > xrange(2)
...
if isnan(newy) 
   ...
   warndlg(getString(message('stats:disttool:WarnDlgProbNum')));
   return;
end
...
if ~iscdf
  ...
  switch ud.dists(popupvalue).rvname
    ...
    case 'burrrv', % Burr
       % 1: alpha 2: c 3: k
        if phi(2) <= 1
           ...
           % Depending on the value of c and k, the
           % maximum PDF value mode could either occur for the lowest
           % or the highest end of the c range
           c = phi(2);
           k = phi(3); % But it always occurs for the largest k
           alpha = plo(1); % and the lowest alpha
           maxy = k./(c.^k .* alpha).*(k.*c +1).^(k + 1./c).*(c-1).^(1-1./c)./(k+1).^(k+1);
        end
    case 'chi2rv', % Chi-square
       tempx = linspace(xrange(1),xrange(2),101);
       maxy = pdf(code,tempx,plo);
    case 'unidrv', % Discrete Uniform
       maxy = 1 ./ plo;
    case 'exprv', % Exponential
       maxy = 1 / plo;
    ...
    case 'ncx2rv', % Noncentral Chi-square
       maxy = 0.4;
    case 'normrv', % Normal
       maxy = pdf(code,0,0,plo(2));
    case 'poissrv', % Poisson
       maxy = pdf(code,[0 plo(1)],plo(1));
    case 'raylrv', % Rayleigh
       maxy = 0.6;  
    case 'trv', % T
       maxy = 0.4;  
...
if iscdf
    ud = changedistribution(iscdf,ud);
    set(ud.yaxistext,'String',getString(message('stats:disttool:LabelProb')));
    ...
    set(ud.yaxistext,'String',getString(message('stats:disttool:LabelDensity')));
...
if badval
    ...
    wmsg = getString(message('stats:disttool:WarnDlgBadLBValue', ...
...
if badval
    ...
    wmsg = getString(message('stats:disttool:WarnDlgBadUBValue', ...
                   fieldentry, preventry));
    uiwait(warndlg(wmsg, 'DISTTOOL', 'modal'))
...
if slidervaluesok(popupvalue, cv, sliderno, ud)
    ...
    wmsg = getString(message('stats:disttool:WarnDlgBadParamValue', ...
