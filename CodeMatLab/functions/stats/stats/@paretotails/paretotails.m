...
%   $Revision: 1.1.8.4 $  $Date: 2011/08/17 21:29:20 $
...
try
  [F,X] = cdfFunc(x);
    ...
    m = message('stats:paretotails:InvalidCDFFUN',func2str(cdfFunc));
...
if any(diffx<=0)
    if diffx(1)==0 && all(diffx(2:end)>0)
        ...
        error(message('stats:paretotails:XfromCDFFUNnotDistinct'));
    end
end
if any(diff(F)<0)
    error(message('stats:paretotails:FfromCDFFUNdecreasing'))
end
...
ws(3) = warning('off','stats:gpfit:ConvergedToBoundary1');
ws(4) = warning('off','stats:gpfit:ConvergedToBoundary2');
    ...
    if pLower > 0                  % Lower tail estimation via maximum likelihood.
        ...
        if length(y)>=2 && max(y)>min(y)
            ...
            if isequal(wid,'stats:gpfit:ConvergedToBoundary') || ...
               isequal(wid,'stats:gpfit:ConvergedToBoundary1') || ...
               isequal(wid,'stats:gpfit:ConvergedToBoundary2')
                warning(message('stats:paretotails:ConvergedToBoundaryLower'));
            elseif isequal(wid,'stats:gpfit:IterOrEvalLimit')
                warning(message('stats:paretotails:NoConvergenceLower'));
            else
            ...
            pLower = 0;
            QL = X(1);
            warning(message('stats:paretotails:NoLowerTailData'));
        end
    ...
    if pUpper < 1                  % Upper tail estimation via maximum likelihood.
        ...
        if length(y)>=2 && max(y)>min(y)
            ...
            if isequal(wid,'stats:gpfit:ConvergedToBoundary') || ...
               isequal(wid,'stats:gpfit:ConvergedToBoundary1') || ...
               isequal(wid,'stats:gpfit:ConvergedToBoundary2')
                warning(message('stats:paretotails:ConvergedToBoundaryUpper'));
            elseif isequal(wid,'stats:gpfit:IterOrEvalLimit')
                warning(message('stats:paretotails:NoConvergenceUpper'));
            else
                lastwarn(oldwmsg,oldwid);
            end
            upParam = parameters;
        else
            warning(message('stats:paretotails:NoUpperTailData'));
            pUpper = 1;
            QU = X(end);
        end
...
if isempty(cdfFunc) || strcmpi(cdfFunc,'ecdf')
   cdfFunc = @smoothECDF;
   descr = getString(message('stats:paretotails:assignment_InterpolatedEmpiricalCdf'));
elseif strcmpi(cdfFunc,'kernel')
   cdfFunc = @(x) ksdensity(x,'function','cdf');
   descr = getString(message('stats:paretotails:assignment_InterpolatedKernelSmoothCdf'));
elseif isa(cdfFunc, 'function_handle')
   descr = getString(message('stats:paretotails:sprintf_Function',func2str(cdfFunc)));
else
   error(message('stats:paretotails:NonFunctionHandle'));
end
end
...
if ~isa(x, 'double') || ~isreal(x) || ~isvector(x)
   error(message('stats:paretotails:VectorRequired'));
end

if ~isscalar(pLower) || (pLower < 0) || (pLower > 1)
   error(message('stats:paretotails:InvalidPL'));
end

if ~isscalar(pUpper) || (pUpper < 0) || (pUpper > 1)
   error(message('stats:paretotails:InvalidPU'));
end

if pLower > pUpper
   error(message('stats:paretotails:InconsistentProbabilities'));
end
end
...
if ~isempty(obj.lowerparams)
    ...
    S(j).description = getString(message('stats:paretotails:sprintf_LowerTailGPD',...
                               sprintf('%g',lowerK),sprintf('%g',lowerSigma)));
end
...
if ~isempty(obj.upperparams)
    ...
    S(j).description = getString(message('stats:paretotails:sprintf_UpperTailGPD',...
                               sprintf('%g',upperK),sprintf('%g',upperSigma)));
end
end
