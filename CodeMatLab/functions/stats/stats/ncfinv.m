...
%   $Revision: 1.1.6.3 $  $Date: 2011/10/15 02:06:33 $

if nargin <  4, 
    error(message('stats:ncfinv:TooFewInputs')); 
end

[errorcode, p, nu1, nu2, delta] = distchck(4,p,nu1,nu2,delta);

if errorcode > 0
    error(message('stats:ncfinv:InputSizeMismatch'));
end
...
if count==count_limit
     warning(message('stats:ncfinv:NotConverge'));
    fprintf([getString(message('stats:ncfinv:LastStep')) num2str(h(:)') '\n']);
end
