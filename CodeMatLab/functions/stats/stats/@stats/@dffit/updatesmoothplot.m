...
%   $Revision: 1.1.8.2 $  $Date: 2010/09/02 13:56:36 $
...
if fit.plot && fit.isgood
   ...
   [c,~,l,w] = dfswitchyard('statgetcolor',ax,'fit',fit);
   m = 'none';
   ...
   if isequal(fit.ftype, 'probplot')
      ...
      [y,~,outwidth] = ksdensity(ydata, x, 'cens',cens, ...
                    'weight',freq,...
   ...
   set(fit.linehandle,'Tag','distfit','UserData',fit);
...
[f,~,outwidth] = ksdensity(y, x, 'cens',cens, 'weight',freq, ...
