...
% $Revision: 1.1.6.3 $  $Date: 2010/10/08 17:29:00 $
...
schema.prop(c, 'version', 'double');               % version number for object definition
...
p=schema.prop(c, 'pcov', 'MATLAB array');   % estimated covariance of params
p=schema.prop(c, 'loglik', 'double');       % log likelihood
p=schema.prop(c, 'resultstext', 'string');  % results
p=schema.prop(c, 'probdist', 'MATLAB array'); % probability distribution object
