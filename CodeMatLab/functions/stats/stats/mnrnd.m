...
%   $Revision: 1.1.8.4 $  $Date: 2011/05/09 01:25:56 $
...
if nargin < 3
    [m,k] = size(p);
elseif ~isscalar(m)
    error(message('stats:mnrnd:NonscalarM'));
else
    [mm,k] = size(p);
    if ~(mm == 1 || mm == m)
        error(message('stats:mnrnd:PSizeMismatch'));
    end
end
if k < 1
    error(message('stats:mnrnd:NoCategories'));
end

[mm,kk] = size(n);
if kk ~= 1
    error(message('stats:mnrnd:NSizeMismatch'));
