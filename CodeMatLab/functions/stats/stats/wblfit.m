...
%   $Revision: 1.1.8.2 $  $Date: 2011/05/09 01:27:15 $

% Illegal data return an error.
if ~isvector(x)
    error(message('stats:wblfit:BadData'));
elseif any(x <= 0)
    error(message('stats:wblfit:BadData'));
end
...
elseif ~isempty(censoring) && ~isequal(size(x), size(censoring))
    error(message('stats:wblfit:CensSizeMismatch'));
end
if nargin < 4 || isempty(freq)
    freq = [];
elseif ~isempty(freq) && ~isequal(size(x), size(freq))
    error(message('stats:wblfit:FreqSizeMismatch'));
end
