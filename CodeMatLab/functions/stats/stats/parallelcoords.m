...
%   $Revision: 1.1.8.6 $ $Date: 2012/04/20 19:46:29 $
...
if ~ishold
    if pcaFlag
        if dfltTckLabs, xlabel(getString(message('stats:parallelcoords:xlable_PrincipalComponent'))); end
        ylabel(getString(message('stats:parallelcoords:ylable_PCScore')));
    else
        if dfltTckLabs, xlabel(getString(message('stats:parallelcoords:xlable_Coordinate'))); end
        ylabel(getString(message('stats:parallelcoords:ylable_CoordinateValue')));
    end
...
if isnumeric(tcklabs) % Numeric vector, default.
    ...
    for i=1:length(tcklabs)
        varnames{i} = getString(message('stats:parallelcoords:ticklabel_Variable',num2str(tcklabs(i))));
    end
...
if isempty(quantile)
    ...
    datatipTxt = {
        ...
        ''...
        getString(message('stats:parallelcoords:text_Observation',num2str(gind))) ...
        };
    if ~isempty(gname)
    ...
    datatipTxt = {
        ...
        ''...
        getString(message('stats:parallelcoords:text_Quantile',num2str(quantile)))...
        };
    if ~isempty(gname)
        datatipTxt{end+1} = getString(message('stats:parallelcoords:text_Group',gname));
    end
