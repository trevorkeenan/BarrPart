...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:24:08 $
...
if isempty(x)
    ...
    return
elseif ~isvector(x)
    error(message('stats:betafit:VectorRequired'));
end
...
if ((xmin < 0) || (xmax > 1))
    error(message('stats:betafit:XOutsideRange'));
elseif abs(xmin - xmax) <= 2*eps(xmax)
    error(message('stats:betafit:NoDistinctX'));
end
