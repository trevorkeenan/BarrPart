...
%NCX2RND Random arrays from the non-central chi-square distribution.
%   R = NCX2RND(V,DELTA) returns an array of random numbers chosen from the
%   non-central chi-square distribution with parameters V and DELTA.  The
...
%   NCX2RND generates values using the sum of a "zero d.f." non-central
...
%   $Revision: 1.1.6.3 $  $Date: 2010/10/08 17:25:40 $

if nargin < 2
    error(message('stats:ncx2rnd:TooFewInputs'));
end

[err, sizeOut] = statsizechk(2,v,delta,varargin{:});
if err > 0
    error(message('stats:ncx2rnd:InputSizeMismatch'));
end
