...
%   $Revision: 1.1.8.4 $  $Date: 2011/07/20 00:09:32 $

if nargin<2
   error(message('stats:statgetbins:BadInput'));
end
...
if nargin < 3
    ...
    if ischar(binspec)
        ...
        case 'nbins' % ecdfhist(F,X,'nbins',M)
            nbins = specval;
        otherwise
            error(message('stats:ecdfhist:BadBinSpec'));
        end
...
case 'ctrs'
    ...
    binwidth = diff(C);
    if any(binwidth<=0)
        error(message('stats:ecdfhist:NotSortedC'));
    end
...
case 'edges'
    ...
    binwidth = diff(edges);
    if any(binwidth<=0)
        error(message('stats:ecdfhist:NotSortedE'));
    end
    if nargout > 2
    ...
    if ~isempty(x) && ((lo < edges(1)) || (edges(end) < hi))
        warning(message('stats:ecdfhist:DataNotSpanned'));
    end
end
