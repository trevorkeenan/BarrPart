...
%   $Revision: 1.1.8.2 $  $Date: 2010/10/08 17:29:19 $
...
if nargin < 3
    ...
    otherwise, error(message('stats:dgammainc:InvalidTailArg'));
    end
end
...
try
   ...
   error(message('stats:dgammainc:InputSizeMismatch'))
end
dy = y;
if nargout > 2
    d2y = y;
...
if any(a(:) < 0)
   error(message('stats:dgammainc:NegativeArg'))
end
