...
%   $Revision: 1.1.8.5 $  $Date: 2012/03/01 02:30:19 $
...
h1=uicontrol(dffig,'units','pixels','Tag','displaytext',...
            'String',getString(message('stats:dfstrings:uicontrol_DisplayType')), 'style','text','Position',p0,...
...
h2=uicontrol(dffig, 'Style','popup','units','pixels','Tag','displaylist',...
            'String',choices,'BackgroundColor',ones(1,3), ...
...
h3=uicontrol(dffig,'units','pixels','Tag','typetext',...
            'String',getString(message('stats:dfstrings:uicontrol_Distribution')), ...
            'Style','text','Position',p0,...
...
strings = {getString(message('stats:dfstrings:button_Data')) ...
           getString(message('stats:dfstrings:button_NewFit')) ...
           getString(message('stats:dfstrings:button_ManageFits')) ...
           getString(message('stats:dfstrings:button_Evaluate')) ...
           getString(message('stats:dfstrings:button_Exclude'))};
tips = {getString(message('stats:dfstrings:button_ImportViewEtc')) ...
        getString(message('stats:dfstrings:button_AddAFittedDistribution')) ...
        getString(message('stats:dfstrings:button_EditViewEtc')) ...
        getString(message('stats:dfstrings:button_EvaluateFitsEtc')) ...
        getString(message('stats:dfstrings:button_DefineRulesEtc'))};
...
fitting = awtinvoke('com.mathworks.toolbox.stats.Fitting', 'getFitting');
awtinvoke(fitting, 'showNewFit');

% ---------------------- callback for Manage Fit button
function cbkmanfit(varargin)
%CBKMANFIT Callback for Manage Fit button

awtinvoke('com.mathworks.toolbox.stats.FitManager', 'showFitManager');
...
awtinvoke('com.mathworks.toolbox.stats.Evaluate', 'showEvaluate(Ljava/lang/String;Ljava/lang/String;)', ftype, xstr);

% ---------------------- callback for Exclude button
function cbkexclude(varargin)
%CBKEXCLUDE Callback for Exclude button

awtinvoke('com.mathworks.toolbox.stats.Exclude', 'showExclude');
