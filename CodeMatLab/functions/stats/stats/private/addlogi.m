...
%ADDLOGI Add the logistic and log-logistic distributions.

%   Copyright 1993-2012 The MathWorks, Inc.
%   $Revision: 1.1.8.4 $  $Date: 2012/03/01 02:30:15 $

j = length(s) + 1;
s(j).name = getString(message('stats:dfittool:NameLogistic'));
s(j).code = 'logistic';
s(j).pnames = {'mu' 'sigma'};
...
j = j + 1;
s(j).name = getString(message('stats:dfittool:NameLogLogistic'));
