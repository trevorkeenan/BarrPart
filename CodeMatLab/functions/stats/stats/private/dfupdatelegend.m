...
%   $Revision: 1.1.8.5 $  $Date: 2011/08/17 21:29:54 $ 
...
for j=1:n
   ...
   if ds.showbounds
      ...
      if ~isempty(hbounds) && ishghandle(hbounds) ...
         ...
         textDataBounds{nextj,1} = [pre getString(message('stats:dfstrings:cellstr_ConfidenceBounds'))];
...
for j=1:length(hFitConf)
   if havebounds(j) && ishghandle(hFitConf(j)) ...
      ...
      textFitBounds{j} = getString(message('stats:dfstrings:sprintf_confidenceBounds',pre,nms{j}));
...
if ~isempty(h)
   try
      ...
      % Store which lines (and text) are to be displayed in the legend,
      % so that other operations (eg, Print to Figure) do not need
      % to recalculate the information.
      dfgetset('legendContents',{h,c});
   catch
   end
end
