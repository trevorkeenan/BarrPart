...
%   $Revision: 1.1.8.3 $  $Date: 2012/05/11 21:03:36 $

if nargin~=5   % minimal error checking to detect a bad call
   error(message('stats:statrunstestprob:BadInput'));
end
if isempty(R)
    p = R;
    return
...
if updown
    ...
    if N<=1 || N>length(rundist)
        error(message('stats:statrunstestprob:BadN'));
    end
    M = rundist{N};
    p = M / sum(M);
    p = p(R);
...
a = gammaln(N+1) - gammaln(n+1) - localgammaln(N-n+1);
