...
%   $Revision: 1.1.8.4 $  $Date: 2011/08/17 21:29:55 $
...
for j=1:nfits
   fj = flist(j);
   newdlist{ndists+j} = getString(message('stats:dfstrings:sprintf_Estimated',fj.distspec.name,fj.name));
   savelist{j} = fj.name;
end
...
choice = 1;
if ~isempty(cursel)
   choices = find(strcmp(cursel,newdlist));
   if numel(choices) == 1
      choice = choices;
   end
end
