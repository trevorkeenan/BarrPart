...
%   $Revision: 1.1.8.4 $  $Date: 2012/03/01 02:30:18 $

j = length(s) + 1;
s(j).name = getString(message('stats:dfittool:NametLocationScale'));
s(j).code = 'tlocationscale';
s(j).pnames = {'mu' 'sigma' 'nu'};
s(j).pdescription = {'location' 'scale' 'degrees of freedom'};
