...
%   $Revision: 1.1.8.8 $  $Date: 2012/04/30 03:10:08 $
...
while(~isempty(ds))
   ...
   if onplot && showlegend
      blkd{end+1} = sprintf('%s(end+1) = %s;',leghname,hlinename);
...
if isequal(ftype,'probplot') && ~simplecase
   if ischar(dtype)
      ...
      blks{end+1} = sprintf(...
          '%s = internal.stats.dfgetdistributions(''%s'');',...
...
codestring = sprintf('%s%%   Creates a plot, similar to the plot in the main distribution fitting\n',codestring);
codestring = sprintf('%s%%   window, using the data that you provide as input.  You can\n',codestring);
codestring = sprintf('%s%%   apply this function to the same data you used with dfittool\n',codestring);
...
for j=1:length(blkc)
   codestring = sprintf('%s%s\n',codestring,xlate(blkc{j}));
end
codestring = sprintf('%s\n',codestring);
for j=1:length(blks)
   codestring = sprintf('%s%s\n',codestring,xlate(blks{j}));
end
codestring = sprintf('%s\n',codestring);
for j=1:length(blkd)
   codestring = sprintf('%s%s\n',codestring,xlate(blkd{j}));
end
codestring = sprintf('%s\n',codestring);
for j=1:length(blkf)
   codestring = sprintf('%s%s\n',codestring,xlate(blkf{j}));
end
codestring = sprintf('%s\n',codestring);
for j=1:length(blke)
   codestring = sprintf('%s%s\n',codestring,xlate(blke{j}));
end
...
%      blkf{end+1} = sprintf('%s = %s;',xgridname,xgridname);
