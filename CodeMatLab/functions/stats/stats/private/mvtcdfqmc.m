...
%   $Revision: 1.1.6.4 $  $Date: 2011/10/15 02:07:30 $

if nargin < 4
    error(message('stats:mvtcdfqmc:NumberOfInputs'));
end
...
if ~all(a < b)
    if any(a > b)
        error(message('stats:mvtcdfqmc:BadLimits'));
...
if any(any(abs(tril(Rho,-1)) > .999))
    warning(message('stats:mvtcdfqmc:HighCorr'));
end
...
err = NaN;
if verbose > 1
    disp(getString(message('stats:mvtcdfqmc:VerboseErrorEstimateHeader1')));
    disp(getString(message('stats:mvtcdfqmc:VerboseErrorEstimateHeader2')));
end
for i = 5:length(P)
    ...
    if verbose > 1
        disp(getString(message('stats:mvtcdfqmc:VerboseErrorEstimateLine',...
                sprintf('%.5g',p), sprintf('%.5e',err), sprintf('%d',funevals))));
    end
    if err < tol
        if verbose > 0
            disp(getString(message('stats:mvtcdfqmc:VerboseSuccessMessage',...
                sprintf('%g',tol), sprintf('%d',funevals))));
        end
        return
    end
end
warning(message('stats:mvtcdfqmc:ErrorTol', num2str( tol ), maxfunevals));
