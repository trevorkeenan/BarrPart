function [pval,pvals]=statctexact(x,wts,tstar,dispopt)
...
%   $Revision: 1.1.8.3 $  $Date: 2011/10/15 02:07:32 $


r = size(x,1);
if (r~=2)
   error(message('stats:statctexact:BadX'));
end
...
if isempty(wts) || isempty(tstar)
   [tstar, wts, expected] = teststat(x, wts);
else
...
if dispopt
   disp(getString(message('stats:statctexact:alignedText_ExactHeader')));
   disp(getString(message('stats:statctexact:alignedText_TestStatistic', ...
       sprintf('%g', tstar))));
   disp(getString(message('stats:statctexact:alignedText_ProbLT', ...
       sprintf('%g',tstar), sprintf('%g',pvals(1)) )));
   disp(getString(message('stats:statctexact:alignedText_ProbLE', ...
       sprintf('%g',tstar), sprintf('%g',pvals(2)+pvals(3)))));
   disp(getString(message('stats:statctexact:alignedText_ProbGT', ...
       sprintf('%g',tstar), sprintf('%g',pvals(3)))));
   disp(getString(message('stats:statctexact:alignedText_2Sided', ...
       sprintf('%g', p2))));
end

% ----------------------------------------------------------
function [tstar, wts, expected] = teststat(x, wts)
...
%    disp([i1 i2 i3]);
   disp(getString(message('stats:statctexact:debugText_Header')));
   disp([num2str(i1) ' ' num2str(i2) ' ' num2str(i3)]);
end
