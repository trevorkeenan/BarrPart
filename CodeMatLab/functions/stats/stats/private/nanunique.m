...
%   $Revision: 1.1.8.2 $  $Date: 2010/10/08 17:29:29 $
...
if nargin > 1
    ...
    if nargin > 2
        if any(options > 1) || (options(2)+options(3) > 1)
            error(message('stats:nanunique:RepeatedFlag'));
        end
    end
    if sum(options) < nargin-1
        error(message('stats:nanunique:UnknownFlag'));
    end
    byrow = (options(1) > 0);
    if options(2) > 0
        order = 'first';
...
elseif nargin == 1
    ...
    error(message('stats:nanunique:NotEnoughInputs'));
end
