...
%   $Revision: 1.1.8.6 $  $Date: 2012/04/20 19:46:37 $
...
l(arg2) = -inf;
if any(u == l) 
   error(message('stats:statsfminbx:NoBounds', 'Equal upper and lower bounds not permitted in this large-scale method.', 'Use equality constraints and the medium-scale method instead.'));
elseif min(u-l) <= 0
...
if strcmp(optimget(options,'DerivativeCheck',defaultopt,'fast'),'on')
   warning(message('stats:statsfminbx:NoDerivativeCheck', 'Trust region algorithm does not currently check user-supplied gradients,', '  ignoring OPTIONS.DerivativeCheck.'));
end
...
while ~ex
   if ~isfinite(val) || any(~isfinite(g))
      error(message('stats:statsfminbx:BadFunctionOutput', funfcn{ 2 }, ' cannot continue: ', 'user function is returning Inf or NaN values.'));
   end
