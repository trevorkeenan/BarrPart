...
% $Revision: 1.1.8.2 $  $Date: 2011/08/17 21:29:40 $
...
switch action
 ...
 case 'editfit'
   ...
   if isequal(htag,'distfit')  % should always be true
      hndl = get(h,'UserData');
      fm = awtinvoke('com.mathworks.toolbox.stats.FitsManager', 'getFitsManager');
      awtinvoke(fm, 'editFit(Ljava/lang/String;)', hndl.name);
   end
   changed = false;
 ...
 case 'binrules'
   ...
   if isequal(htag,'dfdata')
      hndl = get(h,'UserData');
      nm = get(hndl,'name');
      bw = awtinvoke('com.mathworks.toolbox.stats.BinWidth', 'getBinWidth'); % get dialog
      awtinvoke(bw, 'displayBinWidth(Ljava/lang/String;)', nm); % display dialog for this data set
   end
   changed = false;
...
uimenu(cFit,'Label',getString(message('stats:dfstrings:label_Color')),'Tag','color','Callback',@dfdocontext);

% Add menu items for line and marker control
uwidth = uimenu(cFit,'Label',getString(message('stats:dfstrings:label_LineWidth')),'Tag','linewidth');
ustyle = uimenu(cFit,'Label',getString(message('stats:dfstrings:label_LineStyle')),'Tag','linestyle');
umark = uimenu(cFit,'Label',getString(message('stats:dfstrings:label_Marker')),'Tag','marker','Position',2);

% Add menu items to control confidence bounds
uimenu(cFit,'Label',getString(message('stats:dfstrings:label_ConfidenceBounds')),'Callback',@dfdocontext,...
            'Tag','confbounds');
...
uimenu(cFit,'Label',getString(message('stats:dfstrings:label_HideFit')),'Tag','hidecurve','Callback',@dfdocontext,...
       'Separator','on');
uimenu(cFit,'Label',getString(message('stats:dfstrings:label_DeleteFit')),'Tag','deletefit','Callback',@dfdocontext);
uimenu(cFit,'Label',getString(message('stats:dfstrings:label_EditFit')),'Tag','editfit','Callback',@dfdocontext);

% Add items for data menus only
uimenu(cData,'Label',getString(message('stats:dfstrings:label_HideData')),'Tag','hidecurve',...
