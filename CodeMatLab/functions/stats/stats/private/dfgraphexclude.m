...
% $Revision: 1.1.8.5.4.1 $  $Date: 2012/06/22 15:32:05 $ 
...
% We want only Tools and Help menus
h = findall(subfig, 'Type','uimenu', 'Parent',subfig);
htool = [];
hhelp = [];
for j=1:length(h)
   mtag = get(h(j),'Tag');
   if isequal(mtag,'figMenuTools')
       htool = h(j);
   elseif isequal(mtag,'figMenuHelp')
       hhelp = h(j);
   else
       delete(h(j));
   end
...
for j=1:length(h)
   mtag = get(h(j),'Tag');
   if ~ismember(mtag,{'figMenuPan' 'figMenuZoomOut' 'figMenuZoomIn'})
       delete(h(j));
   end
end
h = findall(hhelp, 'Type','uimenu', 'Parent',hhelp);
delete(h);
uimenu(hhelp, 'Label',getString(message('stats:dfstrings:label_StatisticsToolboxHelp')), 'Position',1,'Callback',...
       'doc stats');
uimenu(hhelp, 'Label', getString(message('stats:dfstrings:label_DistributionFittingToolHelp')), 'Position',2,'Callback',...
        @(varargin) dfhelpviewer('distribution_fitting', 'dfittool'));
uimenu(hhelp, 'Label',getString(message('stats:dfstrings:label_GraphicalExclusion')), 'Position',3,'Callback',...
       @(varargin) dfhelpviewer('exclude_data', 'dfittool'));
...
h1(h1==findall(h1,'flat', 'Tag','Exploration.ZoomIn')) = [];
h1(h1==findall(h1,'flat', 'Tag','Exploration.ZoomOut')) = [];
h1(h1==findall(h1,'flat', 'Tag','Exploration.Pan')) = [];
delete(h1);
...
a1=uicontrol(subfig,'Units','pixels','Position',pbutton,...
            ...
            'String',getString(message('stats:dfstrings:button_Close')));
e = get(a1,'Extent');
...
a3 = uicontrol(subfig,'Units','pixels','Position',pbutton,...
              ...
              'String',getString(message('stats:dfstrings:button_RemoveUpperLimit')));
e = get(a3,'Extent');
pbutton(3) = 1.25 * e(3);
pbutton(1) = max(1,pbutton(1) - margin - pbutton(3));
set(a3,'Position',pbutton,'String',getString(message('stats:dfstrings:button_AddUpperLimit')));
...
a4=uicontrol(subfig,'Units','pixels','Position',pbutton,...
            ...
            'String',getString(message('stats:dfstrings:button_AddLowerLimit')));
...
if dolegend
   ...
   a = uicontrol(subfig, 'Units','pixels', 'Position',p,...
                 'Style','text','String',getString(message('stats:dfstrings:uitext_ObservedFilledCircles')));
   e = get(a,'Extent');
   p(3:4) = e(3:4);
   set(a,'Position',p);
   p(2) = p(2) + p(4) + 5;
   uicontrol(subfig, 'Units','pixels', 'Position',p,...
             'Style','text','String',getString(message('stats:dfstrings:uitext_CensoredOpenCircles')));
end
...
if ~isempty(subfig) && ~ishghandle(subfig) ...
...
function fixcursor(obj,eventData)
...
excludePanel = javaMethodEDT('getExcludePanel', 'com.mathworks.toolbox.stats.Exclude');
javaMethodEDT('setGraphExcludeFlag', excludePanel, false);
closereq;
