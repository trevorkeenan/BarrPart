...
%   $Revision: 1.1.8.8 $  $Date: 2011/08/17 21:29:48 $
...
if isempty(fn)
   [fn,pn] = uiputfile(filespec,getString(message('stats:dfstrings:dlg_SaveSession')));
...
try
   ...
   uiwait(errordlg(getString(message('stats:dfstrings:dlg_ErrorSavingSessionFile', ME.message)),...
                   getString(message('stats:dfstrings:dlg_SaveError')),'modal'))
   ok = false;
   return
end
ok = true;
...
if isempty(fn)
   [fn,pn] = uigetfile(filespec,getString(message('stats:dfstrings:dlg_LoadSession')));
   if isequal(fn,0) || isequal(pn,0)
      return
   end
   if ~ismember('.',fn)
...
try
   ...
   uiwait(errordlg(getString(message('stats:dfstrings:dlg_ErrorLoadingSessionFile', ME.message)),...
                   getString(message('stats:dfstrings:dlg_LoadError')),'modal'))
...
for j=1:nrequired
   if ~isfield(s,varnames{j})
      uiwait(errordlg(getString(message('stats:dfstrings:dlg_NotValidSessionFile')),...
                      getString(message('stats:dfstrings:dlg_FileInvalid')),'modal'))
      return
   end
end
...
if ~ismember(s.version,allversions)
   uiwait(errordlg(getString(message('stats:dfstrings:dlg_BadVersionSessionFile')),...
                   getString(message('stats:dfstrings:dlg_InvalidVersion')),'modal'))
   return
end
...
try
   ...
   newdists = mergedistributions(olddists,s.alldists,s.version);
   allnames = {newdists.name};
   [~,sortidx] = sort(allnames);
   newdists = newdists(sortidx);
   dfsetdistributions(dft,newdists);
catch ME
   uiwait(errordlg(getString(message('stats:dfstrings:dlg_ErrorLoadingSavedSession',ME.message)),...
                   getString(message('stats:dfstrings:dlg_FileInvalid')),'modal'));
end
...
value = find(strcmp(s.guistate.ftype, ftypes));
...
% Set selected distribution for probability plots
...
if isprobplot
   ...
   if ischar(s.guistate.dtype)
       value = find(strcmp(s.guistate.dtype, dtypes));
       set(hDistributionList, 'Value', value);
   else
       for j=1:length(s.allfits)
           if isequal(s.guistate.dtype.name,s.allfits(1).name) && ...
              isequal(s.guistate.dtype.params,s.allfits(1).params)
              set(hDistributionList,'Value',length(dtypes)+j);
              break
           end
       end
   end
...
% ----------------------------------
function dnew = mergedistributions(d1,d2,savedversion)
...
if savedversion<=1
    % At one point during version 1, cryptic codes were expanded
    d2 = updatecodes(d2);
end
...
% --------------------------------------
function d = updatecodes(d)

% Get codes from file, and define a set of codes that were expanded from an
% earlier shortened form
codes = {d.code};
short = {'bisa' 'invg' 'logl' 'logi' ...
         'naka' 'rice' 'tls'};
long  = {'birnbaumsaunders' 'inversegaussian' 'loglogistic' 'logistic' ...
         'nakagami' 'rician' 'tlocationscale'};

% Find the codes that need to be expanded 
[tf,loc] = ismember(codes,short);
indices = find(tf);
locs = loc(tf);

% Expand them
for j=1:length(indices)
    d(indices(j)).code = long{locs(j)};
end
