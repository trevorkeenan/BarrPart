...
%   $Revision: 1.1.8.5 $  $Date: 2011/05/09 01:28:16 $
    ...
    function obj=setpieces(obj,p,q,distn)
        ...
        if ~isvector(p) || any(p<=0) || any(p==1) || ~issorted(p) || any(isnan(p))
            error(message('stats:piecewisedistribution:BadP'));
        end
        if ~isvector(q) || any(~isfinite(q)) || ~issorted(q) || numel(q)~=nsegments-1
            error(message('stats:piecewisedistribution:BadQ'));
        end
        if ~isstruct(distn) || numel(distn)~=nsegments
            error(message('stats:piecewisedistribution:BadDistn', nsegments));
        end
        ...
        if any(~fieldexists(1:nrequired))
            ...
            error(message('stats:piecewisedistribution:MissingField', allfields{ j }));
        end
            ...
            for j=1:nsegments
                ...
                if (k<=nrequired) || ~isempty(val)
                    if isa(val,thistype) || isempty(thistype)
                        ...
                        error(message('stats:piecewisedistribution:BadField', j, thisfield, thistype));
                    end
                end
            end
        end
    ...
    methods(Hidden = true, Static = true)
        function a = empty(varargin)
            error(message('stats:piecewisedistribution:NoEmptyAllowed', upper( mfilename )));
        end
    end
...
error(message('stats:piecewisedistribution:NoCatAllowed', upper( mfilename )));
end

function throwUndefinedError()
st = dbstack;
name = regexp(st(2).name,'\.','split');
error(message('stats:piecewisedistribution:UndefinedFunction', name{ 2 }, mfilename));
end
