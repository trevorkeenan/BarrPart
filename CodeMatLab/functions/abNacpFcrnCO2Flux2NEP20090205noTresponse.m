function [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,mtR,mcR,mtGEP,mcGEP,bR,bGEP] = ... 
	abNacpFcrnCO2Flux2NEP20090205noTresponse ... 
	(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTh,FracSEBClosure,cSiteYr,iFig); 


% modifed version of the gap filling routine to remove the T-dependence of
% GPP, tjk, Nov. 2012.
...
%	Assign the moving window parameters. 
	...
	WindowWidthR = 100; WindowIncR = 10; 
	WindowWidthGEP = 100; WindowIncGEP = 10; 
	
	if nRecsPerDay==24; 
		WindowWidthR = 50; WindowIncR = 5; 
		WindowWidthGEP = 50; WindowIncGEP = 5; 
	end; 
...
%	limit data to close to the mode
	...
	mmcR=nanmedian(mcR); qmcR=fcNanIqr(fcx2colvec(mcR)); 
% % % 	clf; hist(mcR); pause; 
...
%	First, estimate the constant bGEP parameters using warm-season data only. 
	...
	bGEPGuess = [20 500]; 
	lasterror('reset');
	bGEP = nlinfit(PPFD(iReg),GEP(iReg),@fcCalcRs2GEP,bGEPGuess); 
	err = lasterror;
	if ~isempty(err.message); 
