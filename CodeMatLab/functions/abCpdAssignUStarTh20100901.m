	function [CpA,nA,tW,CpW,cMode,cFailure,fSelect,sSine,FracSig,FracModeD,FracSelect,xCpSelect] ... 
		= cpdAssignUStarTh20100901(Stats,iFig,cSiteYr) 
...
%		= cpdExtractuStarTh20100901 (Stats,iFig,cSiteYr) 
%
%	where: 
%
...
%	-	iFig is the figure number, set to zero to skip plotting
%		the aggregation analysis
%	-	cSiteYr is text containing site and year for the iFig plot
%
...
%	Functions called: 
%
%		cpdBin, fcDateTick, fcEqnAnnualSine, fcNanIqr, fcReadFields 
...
%	=======================================================================
    ...
    xCpSelect=[];       % tjk, to avoid error when none is selected
...
% 	nWindowsN=4; nSelectN=nWindowsN*nStrataN*nBoot; 
	nWindowsN=1; nSelectN=nWindowsN*nStrataN*nBoot; % relaxed to ensure finding Cp
...
% 	if FracSelect<0.10; cFailure='Less than 10% successful detections. '; return;  end; 
	if FracSelect<0.05; cFailure='Less than 5% successful detections. '; return;  end; % relaxed, tjk.
...
%	Exclude outliers from Select mode based on Cp and regression stats
	...
	if nSelect<nSelectN; cFailure=sprintf('Too few selected change points: %g/%g',nSelect,nSelectN); xCpSelect=NaN*xCp;return;  end; 
