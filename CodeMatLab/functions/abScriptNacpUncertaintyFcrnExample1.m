...
%	========================================================================
	...
	DirRoot='../'; 
	DirRoot2='/Users/trevorkeenan/Results/NOAA/UncertaintyBARR_TK/';
	mkdir([DirRoot2 'Output/FCRN/']); DirOut=[DirRoot 'Output/FCRN/']; 
	mkdir([DirRoot 'Data_L2/']); DirL2=[DirRoot 'Data_L2/']; 
	mkdir([DirRoot2 'figuresQC/']); DirQC=[DirRoot 'figuresQC/']; 
	mkdir([DirRoot2 'log/']); DirLog=[DirRoot 'Log/']; 

	mkdir([DirOut2 'randomUncertainty/']); 
	mkdir([DirOut2 'uStarThAnalysis/']); 
	mkdir([DirOut2 'uStarTh2NeeUncertainty/']); 
	
	mkdir([DirOut2 'randomUncertainty/figures/']); 
	mkdir([DirOut2 'uStarThAnalysis/figures/']); 
	mkdir([DirOut2 'uStarTh2NeeUncertainty/figures/']); 

%	========================================================================

	cSite='UNkwn'; cYr='9999'; cSiteYr=[cSite '-' cYr]; 
	
	FileDiary=[DirLog cSiteYr '.log']; diary(FileDiary); 

	FileL2='data4alan.mat'; % normally derived from cSite and cYr
...
%	check processing at mean uStarTh. 
	...
	FracSEBClosure=1; muStarTh=mean(uStarThs,2); iFig=104; 
...
% 	calc uStarTh-related uncertainties in NEE, RE and GPP
    ...
    [uuEnsemble,uuNEE] = ... 
		abNacpFcrnUStarTh2NepUncertainty20100423 ... 
		(t,NEE,uStar,PPFD,Ta,Ts,PPFD,Ta,Ts,fNight,uStarThs,cSiteYr,iFig); 
...
% % % 	uStarTh=mean(CpBoot); 
	...
	uStarTh=CpSine; nMC=1e1; iFig=106; % nMC is the # MonteCarlo reps
    % nMC usually set to 1000 but will take a long time to run, tjk.
    
	[ruEnsemble,ruNEE]=abNacpFcrnRandomUncertainty20100423 ...
		(t,NEE,uStar,PPFD,Ta,Ts,fNight,uStarTh,DirOut,cSiteYr,nMC,iFig); 
		
	eval(['save ' DirOut 'randomUncertainty/randomUncertaintyFcrn_' cSiteYr ... 
		' cSite cYr ruEnsemble ruNEE;']);
 
	if iFig>0; 
		eval(['print -djpeg100 ' DirOut 'randomUncertainty/figures/randomUncertaintyFcrn_' cSiteYr ';']); 
	end; 
